...
def appendDtectArgs( cmd, args=None ):
  ...
  ----------
  cmd : list
        List to which the returned elements will be added
  arg : dict, optional
        Dictionary with the members 'dtectdata' and 'survey' as 
        single element lists, and/or 'dtectexec' (see odpy.getODSoftwareDir)
  ...
  -------
  list
    Input list with the added data root and survey directory name if provided
...
def getPythonCommand(scriptfile,posargs=None,dict=None,args=None):
  ...
  dict : str, optional
         Value for the argument with key "--dict":
...
def execCommand( cmd, background=False ):
  ...
  ----------
  cmd : list
        command to be executed with its arguments.
  ...
  or
  A psutil.Popen object (background=True)
...
def printProcessTime(procnm,isstart,print_fn=std_msg,withprocline=True):
  ...
  procnm : str
            Process name
  isstart : bool
            Status: Start or end of this process.
