...
public:
    ...
    uiString	message() const			{ return tr("Copying values"); }
    uiString	nrDoneText() const		{ return sPositionsDone(); }
...
public:
    ...
    uiString	message() const		{ return tr("Replacing values"); }
    uiString	nrDoneText() const		{ return sPositionsDone(); }
...
template <class T>
inline T* memValueSet( T* ptr, T val, od_int64 size )
{
    if ( size < cMinMemValSetParallelSize )
	return sysMemValueSet( ptr, val, size );

    MemSetter<T> msetter( ptr, val, size );
    msetter.execute();
