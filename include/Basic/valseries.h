...
/*!\brief Gets data from ValueSeries. */
...
ValueSeriesGetAll( const ValueSeries<T>& from, ValueSeries<T>& to,
			od_int64 nrelements )
    : from_( from )
    , to_( &to )
    , toptr_( 0 )
...
ValueSeriesGetAll(const ValueSeries<T>& from, T* to, od_int64 nrelements )
    : from_( from )
    , toptr_( to )
    , to_( 0 )
    , nrelements_( nrelements )
