...
________________________________________________________________________
 ...
 Date:          April 2005
 RCS:           $Id: array2dfilter.h,v 1.1 2006-11-17 12:17:16 cvsbert Exp $
________________________________________________________________________
...
public:
    ...
    inline const Stats::RunCalc& calc() const	{ return *calc_; }

    inline int		nextStep();
    inline const char*	message() const		{ return "Filtering data"; }
    inline int		nrDone() const		{ return nrdone_; }
    ...
    , nrdone_(0)
{
    if ( rowsize_ < 2 * p.stepout_.r() + 1
      ...
      || (pars_.stepout.c() < 1 && pars_.stepout.r() < 1) )
	{ nrdone_ = colsize_; return; }

    buff_ = new T* [ nrcols_ ];
    idxs_ = new int [ nrcols_ ];
    for ( int idx=0; idx<nrcols_; idx++ )
    {
	buff_[idx] = new T [rowsize_];
	idxs_[idx] = idx;
	int inpcol = idx < pars_.stepout.c() ? 0 : idx - pars_.stepout.c();
	if ( inpcol >= colsize_ ) inpcol = colsize_ - 1;
	for ( int irow=0; irow<rowsize_; irow++ )
	    buff_[idx][irow] = arr_->get( irow, inpcol );
    }
    if ( !buff_[nrcols_-1] )
	{ releaseAll(); ErrMsg("Memory full"); nrdone_ = colsize_; return; }

    Stats::RunCalcSetup setup( !mIsUdf(pars_.rowdist_) );
    setup.require( pars_.type_ );
    calc_ = new Stats::RunCalc( setup );
}


template <class T> inline
void Array2DFilterer<T>::~Array2DFilterer()
{
    releaseAll();
}
...
void Array2DFilterer<T>::releaseAll()
{
    ...
    for ( int idx=0; idx<nrcols_; idx++ )
	delete [] buff_[idx];
    delete [] buff_; buff_ = 0;
}


template <class T> inline int Array2DFilterer<T>::nextStep()
{
    if ( !buff_ || nrdone_ >= colsize_ )
	return Executor::Finished;

    if ( !manageBufs(nrdone_) )
	return Executor::Finished;
...
template <class T> inline void Array2DFilterer<T>::manageBufs( int col )
{
...
inline void Array2DFilterer<T>::doPoint( int row )
{
	...
	arr_.set( irow, realcol, calc_->getValue(pars_.type_) );
    }
}
