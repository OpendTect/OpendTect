...
list( APPEND OD_MODULE_RUNTIMEPATH "${OD_BINARY_BASEDIR}/${OD_RUNTIME_DIRECTORY}" )
list( APPEND OD_MODULE_RUNTIMEPATH "${CMAKE_BINARY_DIR}/${OD_RUNTIME_DIRECTORY}" )
if ( OD_CREATE_LAUNCHERS AND NOT "${OD_SUBSYSTEM}" STREQUAL "od" )
    set( LAUNCHERS_ENV "DTECT_APPL=${OD_BINARY_BASEDIR}\nOD_USER_PLUGIN_DIR=\${CMAKE_BINARY_DIR\}" )
endif()
...
if ( OD_MODULE_TESTPROGS OR OD_MODULE_PROGS OR OD_MODULE_GUI_PROGS OR OD_ELEVATED_PERMISSIONS_PROGS OR OD_ELEVATED_PERMISSIONS_GUI_PROGS )
	...
	if ( OD_MODULE_COMPILE_DEFINITIONS )
	    list( APPEND OD_PROG_COMPILE_DEFINITIONS "${OD_MODULE_COMPILE_DEFINITIONS}" )
	endif()
	if ( OD_MODULE_LINK_OPTIONS )
	    list( APPEND OD_PROG_LINK_OPTIONS "${OD_MODULE_LINK_OPTIONS}" )
	endif()
	...
	if ( EXEC IN_LIST OD_MODULE_TESTPROGS )
	    ...
	    list( APPEND OD_PROG_COMPILE_DEFINITIONS __testprog__ )
	else()
	    if ( DEFINED OD_FOLDER )
	...
	if ( WIN32 AND OD_ENABLE_BREAKPAD AND NOT "${OD_EXEC_GUI_SYSTEM}" STREQUAL "" AND
	    ...
	    list ( APPEND OD_PROG_LINK_OPTIONS "$<$<NOT:$<CONFIG:Debug>>:/ignore:4099>" )
	endif()

	ADD_TARGET_PROPERTIES( ${TARGET_NAME} )
	unset( OD_PROG_COMPILE_DEFINITIONS )
	unset( OD_PROG_LINK_OPTIONS )
	...
	target_link_libraries(
		...
		RUNTIME_LIBRARY_DIRS "${OD_MODULE_RUNTIMEPATH}"
...
if( OD_MODULE_BATCHPROGS )
	...
	list ( APPEND OD_PROG_COMPILE_DEFINITIONS __prog__ )
...
foreach ( TEST_FILE ${OD_TEST_PROGS} ${OD_BATCH_TEST_PROGS} ${OD_NIGHTLY_TEST_PROGS} )
	...
	if ( IS_DIRECTORY "${OD_TESTDATA_DIR}" AND
	     ((${TEST_FILE} IN_LIST OD_BATCH_TEST_PROGS) OR
	      (${TEST_FILE} IN_LIST OD_NIGHTLY_TEST_PROGS)) )
...
macro( ADD_TARGET_PROPERTIES TARGETNM )
    ...
    if ( OD_PROG_COMPILE_DEFINITIONS )
	list( REMOVE_DUPLICATES OD_PROG_COMPILE_DEFINITIONS )
	target_compile_definitions( ${TARGETNM} PRIVATE
				    ${OD_PROG_COMPILE_DEFINITIONS} )
    elseif ( OD_MODULE_COMPILE_DEFINITIONS )
	list( REMOVE_DUPLICATES OD_MODULE_COMPILE_DEFINITIONS )
	target_compile_definitions( ${TARGETNM} PRIVATE
    ...
    if ( OD_PROG_LINK_OPTIONS )
	list( REMOVE_DUPLICATES OD_PROG_LINK_OPTIONS )
	target_link_options( ${TARGETNM} PRIVATE
			     ${OD_PROG_LINK_OPTIONS} )
    elseif ( OD_MODULE_LINK_OPTIONS )
