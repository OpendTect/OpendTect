...
set ( OD_SEIS_EXECS od_cbvs_browse od_copy_seis od_merge_seis od_process_2dto3d od_process_segyio )
...
macro( OD_ADD_EXTERNALS )
    ...
    OD_ADD_BREAKPAD()
    OD_FIND_ZLIB()
    if ( UNIX ) #Keep before Qt and before osg(Geo)
	OD_FIND_PNG()
	OD_FIND_Freetype()
	OD_FIND_Fontconfig()
    endif()
...
function(od_get_dll _lib _dll)
	...
	if ( NOT EXISTS "${DLLFILENM}" )
	    file( GLOB DLLFILENM "${DLLPATH}/${BASEFILENM}lib.dll" )
	endif()
	unset( DLLPATH )
	unset( BASEFILENM )
	if ( EXISTS "${DLLFILENM}" )
...
function( od_install_unknown_library _trgt _dest )

    get_target_property( _configs ${_trgt} IMPORTED_CONFIGURATIONS )
    if ( _configs )
	get_target_property( _loc_release ${_trgt} IMPORTED_LOCATION_RELEASE )
	if ( UNIX )
	    get_target_property( _soname_release ${_trgt} IMPORTED_SONAME_RELEASE )
	endif()
	if ( NOT _loc_release )
	    get_target_property( _loc_release ${_trgt} IMPORTED_LOCATION_RELWITHDEBINFO )
	    if ( UNIX )
		get_target_property( _soname_release ${_trgt} IMPORTED_SONAME_RELWITHDEBINFO )
	    endif()
	endif()
	if ( NOT _loc_release )
	    get_target_property( _loc_release ${_trgt} IMPORTED_LOCATION_MINSIZEREL )
	    if ( UNIX )
		get_target_property( _soname_release ${_trgt} IMPORTED_SONAME_MINSIZEREL )
	    endif()
	endif()
	get_target_property( _loc_debug ${_trgt} IMPORTED_LOCATION_DEBUG )
	if ( UNIX )
	    get_target_property( _soname_debug ${_trgt} IMPORTED_SONAME_DEBUG )
	endif()
	if ( NOT _loc_release OR NOT _loc_debug )
	    if ( _loc_release )
	    ...
	    install( PROGRAMS "${_loc}"
		     DESTINATION "${_dest}" )
	    if ( UNIX )
		if ( _soname_release )
		    set( _soname ${_soname_release} )
		else()
		    set( _soname ${_soname_debug} )
		endif()
		if ( APPLE )
		    string( REPLACE "\@rpath/" "" _soname "${_soname}" )
		endif()
		cmake_path( GET _loc PARENT_PATH _locdir )
		install( PROGRAMS "${_locdir}/${_soname}"
			 DESTINATION "${_dest}" )
	    endif()
	else()
	    if ( WIN32 )
		od_get_dll( "${_loc_release}" _loc_release )
		od_get_dll( "${_loc_debug}" _loc_debug )
	    endif()
	    install( PROGRAMS "${_loc_release}"
		     DESTINATION "${_dest}"
		     CONFIGURATION RELEASE;RELWITHDEBINFO;MINSIZEREL )
	    install( PROGRAMS "${_loc_debug}"
		     DESTINATION "${_dest}"
		     CONFIGURATION DEBUG )
	    if ( UNIX )
		cmake_path( GET _loc_release PARENT_PATH _locdir_relase )
		if ( APPLE )
		    string( REPLACE "\@rpath/" "" _soname_release "${_soname_release}" )
		endif()
		install( PROGRAMS "${_locdir_release}/${_soname_release}"
			 DESTINATION "${_dest}"
			 CONFIGURATIONS RELEASE;RELWITHDEBINFO;MINSIZEREL )
		cmake_path( GET _loc_debug PARENT_PATH _locdir_debug )
		if ( APPLE )
		    string( REPLACE "\@rpath/" "" _soname_debug "${_soname_debug}" )
		endif()
		install( PROGRAMS "${_locdir_debug}/${_soname_debug}"
			 DESTINATION "${_dest}"
			 CONFIGURATIONS DEBUG )
	    endif()
	endif()
    else()
	get_target_property( _loc ${_trgt} IMPORTED_LOCATION )
	if ( NOT _loc )
	    get_target_property( _loc ${_trgt} IMPORTED_LOCATION_RELEASE )
	endif()
	if ( NOT _loc )
	    get_target_property( _type ${_trgt} TYPE )
	    if ( NOT ${_type} STREQUAL "INTERFACE_LIBRARY" )
		message( WARNING "Cannot install runtime library for the interface library ${_trgt}" )
	    endif()
	    unset( _type )
	    return()
	endif()
	...
	install( PROGRAMS "${_loc}"
		 DESTINATION "${_dest}" )
	if ( UNIX )
	    get_target_property( _soname ${_trgt} IMPORTED_SONAME )
	    if ( NOT _soname )
		get_target_property( _soname ${_trgt} IMPORTED_SONAME_RELEASE )
	    endif()
	    if ( NOT _soname )
		message( WARNING "Cannot install soname runtime library for the target ${_trgt}" )
		return()
	    endif()
	    if ( APPLE )
		string( REPLACE "\@rpath/" "" _soname "${_soname}" )
...
endfunction( od_install_unknown_library )
...
macro( OD_INSTALL_IMPORTED_LINK_DEPS TRGT )
	...
	if ( TARGET ${_imported_trgt} )
		...
		unset( _INSTDIR )
	    elseif ( (${TARGET_TYPE} STREQUAL "UNKNOWN_LIBRARY") OR
		     (UNIX AND ${TARGET_TYPE} STREQUAL "INTERFACE_LIBRARY") )
		set( _INSTDIR )
		if ( UNIX AND NOT APPLE )
		    get_install_dir( ${_imported_trgt} _INSTDIR ON )
		endif()
		od_install_unknown_library( ${_imported_trgt} "${OD_LOCATION_DIRECTORY}${_INSTDIR}" )
		unset( _INSTDIR )
	    endif()
	    unset( TARGET_TYPE )
	endif()
...
macro( OD_INSTALL_DEPENDENCIES MOD_DEPS )
	...
	elseif ( (${TARGET_TYPE} STREQUAL "UNKNOWN_LIBRARY") OR
	    ...
	    od_install_unknown_library( ${TRGT} "${OD_LOCATION_DIRECTORY}${_INSTDIR}" )
	    unset( _INSTDIR )
	endif()
	unset( TARGET_TYPE )
    endforeach()
...
function( set_target_location _trgt )
    get_target_property( trgt_location ${_trgt} IMPORTED_LOCATION )
    if ( trgt_location AND EXISTS "${trgt_location}" )
	get_filename_component( trgt_imported_location "${trgt_location}" REALPATH )
	set_target_properties( ${_trgt} PROPERTIES
		IMPORTED_LOCATION "${trgt_imported_location}" )
    endif()
    get_target_property( trgt_configs ${_trgt} IMPORTED_CONFIGURATIONS )
    foreach( _config ${trgt_configs} )
	get_target_property( trgt_location ${_trgt} IMPORTED_LOCATION_${_config} )
	if ( trgt_location AND EXISTS "${trgt_location}" )
	    get_filename_component( trgt_imported_location "${trgt_location}" REALPATH )
	    set_target_properties( ${_trgt} PROPERTIES
		    IMPORTED_LOCATION_${_config} "${trgt_imported_location}" )
	endif()
    endforeach()
endfunction( set_target_location)

function( set_target_soname _trgt )
    get_target_property( trgt_soname ${_trgt} IMPORTED_SONAME )
    if ( NOT trgt_soname )
	get_target_property( trgt_location ${_trgt} IMPORTED_LOCATION )
	if ( trgt_location AND EXISTS "${trgt_location}" )
	    set( trgt_soname "${trgt_location}" )
	    if ( APPLE )
		cmake_path( GET trgt_soname STEM trgt_soname_last_stem )
		set( trgt_soname "@rpath/${trgt_soname_last_stem}.dylib" )
	    else()
		cmake_path( GET trgt_soname STEM LAST_ONLY trgt_soname_last_stem_new )
		if ( "${trgt_soname_last_stem_new}" MATCHES "\\.so$" )
		    cmake_path( GET trgt_soname FILENAME trgt_soname )
		else()
		    while( NOT ${trgt_soname} STREQUAL "" )
			cmake_path( GET trgt_soname STEM LAST_ONLY trgt_soname_last_stem )
			set( trgt_soname ${trgt_soname_last_stem} )
			cmake_path( GET trgt_soname STEM LAST_ONLY trgt_soname_last_stem_new )
			if ( "${trgt_soname_last_stem_new}" MATCHES "\\.so$" )
			    cmake_path( GET trgt_location PARENT_PATH trgt_loc_dir )
			    if ( NOT EXISTS "${trgt_loc_dir}/${trgt_soname}" )
				set( trgt_soname "${trgt_soname_last_stem_new}.0" )
			    endif()
			    break()
			endif()
		    endwhile()
	    ...
	    set_target_properties( ${_trgt} PROPERTIES
		    IMPORTED_SONAME ${trgt_soname} )
	endif()
    endif()
    unset( trgt_soname )
    get_target_property( trgt_soname ${_trgt} IMPORTED_SONAME_RELEASE )
    if ( NOT trgt_soname )
	get_target_property( trgt_configs ${_trgt} IMPORTED_CONFIGURATIONS )
	foreach( _config ${trgt_configs} )
	    get_target_property( trgt_location_config ${_trgt} IMPORTED_LOCATION_${_config} )
	    if ( trgt_location_config AND EXISTS "${trgt_location_config}" )
		set( trgt_soname "${trgt_location_config}" )
		if ( APPLE )
		    cmake_path( GET trgt_soname STEM trgt_soname_last_stem )
		    set( trgt_soname "@rpath/${trgt_soname_last_stem}.dylib" )
		else()
			...
			while( NOT ${trgt_soname} STREQUAL "" )
			    ...
			    cmake_path( GET trgt_soname STEM LAST_ONLY trgt_soname_last_stem_new )
			    if ( "${trgt_soname_last_stem_new}" MATCHES "\\.so$" )
				cmake_path( GET trgt_location_config PARENT_PATH trgt_loc_dir )
				if ( NOT EXISTS "${trgt_loc_dir}/${trgt_soname}" )
				    set( trgt_soname "${trgt_soname_last_stem_new}.0" )
				endif()
				break()
		...
		set_target_properties( ${_trgt} PROPERTIES
			IMPORTED_SONAME_${_config} ${trgt_soname} )
	    endif()
	endforeach()
    endif()
endfunction()
...
function(OD_MACOS_ADD_EXTERNAL_LIBS _libs_var)
	...
	if ( TARGET ZLIB::ZLIB )
	    list( APPEND _updated_libs ZLIB::ZLIB )
	endif()
    endif()
    if( APPLE AND "Qt${QT_VERSION_MAJOR}::PrintSupport" IN_LIST _updated_libs )
	if ( TARGET Cups::Cups )
	    list( APPEND _updated_libs Cups::Cups )
	endif()
    endif()
...
function( get_thirdparty_libs _deps _var )
	...
	if ( WIN32 )
		...
		get_target_property( _loc ${_dep} IMPORTED_LOCATION )
		if ( EXISTS "${_loc}" )
		    od_get_dll( "${_loc}" _loc )
		    if ( EXISTS "${_loc}" )
			cmake_path( GET _loc FILENAME _loc )
		...
		if ( TARGET_TYPE )
		    ...
		    elseif ( ("${TARGET_TYPE}" STREQUAL "UNKNOWN_LIBRARY" ) OR
			     (UNIX AND "${TARGET_TYPE}" STREQUAL "INTERFACE_LIBRARY") )
			if ( WIN32 )
			    get_target_property( _linklibfile ${_dep} IMPORTED_LOCATION )
			    if ( EXISTS "${_linklibfile}" )
				od_get_dll( "${_linklibfile}" runtimelib )
				if ( EXISTS "${runtimelib}" )
				    list( APPEND _libs "${runtimelib}" )
				endif()
			    endif()
			else()
			    set( _INSTDIR )
			    ...
			    get_target_property( _depconfigs ${_dep} IMPORTED_CONFIGURATIONS )
			    if ( _depconfigs )
				get_filename_component( _libfile "$<TARGET_PROPERTY:${_dep},IMPORTED_SONAME_RELEASE>" NAME )
			    else()
				get_filename_component( _libfile "$<TARGET_PROPERTY:${_dep},IMPORTED_SONAME>" NAME )
			    endif()
			    unset( _depconfigs )
			    list( APPEND _libs "${_INSTDIR}${_libfile}" )
			    unset( _libfile )
			    unset( _INSTDIR )
			endif()
		    endif()
