...
macro( OD_ADD_EXTERNALS )
	...
	if ( WIN32 )
	    string( REPLACE "/DNDEBUG" "/Z7" CMAKE_EXT_CXX_FLAGS_RELEASE "${CMAKE_EXT_CXX_FLAGS_RELEASE}" )
	    set( CMAKE_EXT_LINKER_FLAGS_RELEASE "/debug /INCREMENTAL" )
	else()
	    set( CMAKE_EXT_CXX_FLAGS_RELEASE "${CMAKE_EXT_CXX_FLAGS_RELEASE} -g" )
	endif()
    endif()
    OD_ADD_BREAKPAD()
    OD_ADD_QT()
...
function(guess_runtime_library_dirs _var)
	...
	foreach(_libdir ${_libdirs})
	    ...
	    if ( IS_DIRECTORY "${_dlldir}" )
		list( APPEND _dlldirs "${_dlldir}" )
	    elseif ( IS_DIRECTORY "${_dlldir}/../../bin/Debug" AND
		     _libdir MATCHES ".*Debug.*" )
		get_filename_component( _DLLDIR "${_dlldir}/../../bin/Debug" REALPATH )
		list( APPEND _dlldirs "${_DLLDIR}" )
		unset( _DLLDIR )		
	    else()
...
function(guess_extruntime_library_dirs _var)
	...
	if ( EXISTS "${LIBLOC}" )
		...
		elseif ( IS_DIRECTORY "${DLLDIR}/../../bin/Debug" AND
			 LIBLOC MATCHES ".*Debug.*" )
		    get_filename_component( RUNTIMEDIR "${DLLDIR}/../../bin/Debug" REALPATH )
		else()
...
function( read_soname SOFILENM LIBFILE )
    if ( IS_SYMLINK "${LIBFILE}" )
        execute_process( COMMAND readelf -d "${LIBFILE}"
                         RESULT_VARIABLE STATUS
                         OUTPUT_VARIABLE READELF_OUT
                         OUTPUT_STRIP_TRAILING_WHITESPACE )
        if ( ${STATUS} EQUAL "0" )
            string( REPLACE "\n" ";" READELF_LINES "${READELF_OUT}" )
            foreach( READELF_LINE ${READELF_LINES} )
                if ( NOT "${READELF_LINE}" MATCHES "SONAME" )
                    continue()
                endif()
                string( FIND "${READELF_LINE}" "[" READELF_SUBLINE_POS )
                string( SUBSTRING "${READELF_LINE}" ${READELF_SUBLINE_POS} -1 READELF_SUBLINE )
                string( REPLACE "[" "" READELF_SUBLINE "${READELF_SUBLINE}" )
                string( REPLACE "]" "" READELF_SUBLINE "${READELF_SUBLINE}" )
                set( RESFILENM "${READELF_SUBLINE}" )
                break()
            endforeach()
        else()
            file( READ_SYMLINK "${LIBFILE}" RESFILENM )
        endif()
        unset( READELF_OUT )
    else()
        get_filename_component( RESFILENM "${LIBFILE}" NAME )
    endif()
    set( ${SOFILENM} ${RESFILENM} PARENT_SCOPE )
endfunction( read_soname )
...
macro( OD_GET_WIN32_INST_VARS )
	...
	get_filename_component( DLLBASEPATH ${FILEPATH} DIRECTORY )
...
macro( OD_GET_UNIX_INST_VARS )
	...
	if ( DEFINED LIBSONAME AND LIBSONAME )
		...
		read_soname( FILENAME "${SOURCEFILE}" )
		if ( EXISTS "${FILENAME}")
...
function( add_fontconfig OUTPUT_LIST INPUT_LIST )
    ...
    if ( IS_DIRECTORY "${Fontconfig_ROOT}" )
	get_filename_component( LIBSEARCHPATHS "${Fontconfig_ROOT}/lib" REALPATH )
    elseif ( IS_DIRECTORY "$ENV{FONTCONFIG_DIR}" )
	get_filename_component( LIBSEARCHPATHS "$ENV{FONTCONFIG_DIR}/lib" REALPATH )
    elseif ( IS_DIRECTORY "${FONTCONFIG_DIR}" )
	get_filename_component( LIBSEARCHPATHS "${FONTCONFIG_DIR}/lib" REALPATH )
    endif()
    if ( APPLE )
    ...
    if ( IS_DIRECTORY "${Freetype_ROOT}" )
	get_filename_component( LIBSEARCHPATHS "${Freetype_ROOT}/lib" REALPATH )
    elseif ( IS_DIRECTORY "$ENV{FREETYPE_DIR}" )
	get_filename_component( LIBSEARCHPATHS "$ENV{FREETYPE_DIR}/lib" REALPATH )
    elseif ( IS_DIRECTORY "${FREETYPE_DIR}" )
	get_filename_component( LIBSEARCHPATHS "${FREETYPE_DIR}/lib" REALPATH )
    endif()
    if ( APPLE )
    ...
    if ( IS_DIRECTORY "${PNG_INCLUDE_DIR}" )
	get_filename_component( LIBSEARCHPATHS "${PNG_INCLUDE_DIR}/../lib" REALPATH )
    elseif ( IS_DIRECTORY "${PNG_ROOT}" )
	get_filename_component( LIBSEARCHPATHS "${PNG_ROOT}/lib" REALPATH )
    endif()
    if ( APPLE )
