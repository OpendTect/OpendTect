...
function( od_get_library_filename LIBNM OD_LIBFNM )
    set( LIBFNM "${OD_LOCATION_DIRECTORY}/${SHLIB_PREFIX}${LIBNM}$<$<CONFIG:Debug>:${CMAKE_DEBUG_POSTFIX}>.${SHLIB_EXTENSION}" )
...
if ( DEFINED CMAKE_EXPORT_COMPILE_COMMANDS AND NOT CMAKE_EXPORT_COMPILE_COMMANDS STREQUAL "" )
    set( OD_DEFAULT_CREATE_COMPILE_DATABASE "${CMAKE_EXPORT_COMPILE_COMMANDS}" )
else()
    if ( CMAKE_GENERATOR MATCHES "Ninja" OR CMAKE_GENERATOR MATCHES "Makefile" )
	set( OD_DEFAULT_CREATE_COMPILE_DATABASE "ON" )
    else()
	set( OD_DEFAULT_CREATE_COMPILE_DATABASE "OFF" )
    endif()
endif()
OPTION ( OD_CREATE_COMPILE_DATABASE "Create compile_commands.json database for analyser tools to use"
	 ${OD_DEFAULT_CREATE_COMPILE_DATABASE} )
...
macro( GET_OD_BASE_EXECUTABLES )
	...
	if ( "${MODULE}" MATCHES "^ui" OR "${MODULE}" MATCHES "^vis" )
	    ...
	    list( APPEND OD_CORE_MODULES ${MODULE} )
	    list( APPEND OD_CORE_EXECUTABLES ${OD_${MODULE}_PROGS} )
	endif()
    endforeach()
    list( REMOVE_ITEM OD_CORE_MODULES AllNonUi )
    foreach(  MODULE ${OD_PLUGINS} )
	if ( "${MODULE}" MATCHES "^ui" )
	    list( APPEND OD_GUI_PLUGINS ${MODULE} )
	    list( APPEND OD_GUI_EXECUTABLES ${OD_${MODULE}_PROGS} )
	else()
	    list( APPEND OD_CORE_PLUGINS ${MODULE} )
	    list( APPEND OD_CORE_EXECUTABLES ${OD_${MODULE}_PROGS} )
	endif()
    endforeach()
    foreach(  MODULE ${OD_SPECPROGS} )
	if ( "${MODULE}" MATCHES "^ODui" )
	    list( APPEND OD_GUI_SPECMODS ${MODULE} )
	    list( APPEND OD_GUI_GUIEXECUTABLES ${OD_${MODULE}_PROGS} )
	else()
	    list( APPEND OD_CORE_SPECMODS ${MODULE} )
	    list( APPEND OD_CORE_EXECUTABLES ${OD_${MODULE}_PROGS} )
	endif()
    endforeach()
endmacro()
...
function( _od_json_toggle_instring json idx instring_var )
    string( SUBSTRING "${json}" ${idx} 1 _c )
    if( NOT _c STREQUAL "\"" )
	return()
    endif()
    set( _bslashcount 0 )
	...
	string( SUBSTRING "${json}" ${_j} 1 _cj )
	if( NOT _cj STREQUAL "\\" )
	    break()
	endif()
	math( EXPR _bslashcount "${_bslashcount} + 1" )
	math( EXPR _j "${_j} - 1" )
    endwhile()
    math( EXPR _odd "${_bslashcount} % 2" )
    if( _odd )
	return()
    endif()
    if( ${instring_var} )
	set( ${instring_var} FALSE PARENT_SCOPE )
    else()
	set( ${instring_var} TRUE PARENT_SCOPE )
    endif()
endfunction( _od_json_toggle_instring )

function( od_json_compact input outvar )
    set( _out "" )
    set( _instring FALSE )
	...
	string( SUBSTRING "${input}" ${_i} 1 _c )
	_od_json_toggle_instring( "${input}" ${_i} _instring )
	if( _instring )
...
endfunction( od_json_compact )

function( od_json_format input outvar )
    set( _out "" )
    set( _indent 0 )
    set( _instring FALSE )
	...
	if( NOT _instring )
		...
		if( _next LESS _len )
		    string( SUBSTRING "${input}" ${_next} 1 _nextc )
		    if( ( _c STREQUAL "{" AND _nextc STREQUAL "}" )
			 OR ( _c STREQUAL "[" AND _nextc STREQUAL "]" ) )
			string( APPEND _out "${_c}${_nextc}" )
			set( _skip_next TRUE )
			continue()
		    endif()
		endif()
		string( APPEND _out "${_c}\n" )
		math( EXPR _indent "${_indent} + 1" )
		math( EXPR _spaces "${_indent} * 2" )
		string( REPEAT " " ${_spaces} _pad )
		string( APPEND _out "${_pad}" )
		continue()
	    endif()
	    if( _c STREQUAL "}" OR _c STREQUAL "]" )
		string( APPEND _out "\n" )
    ...
    endforeach()
    set( ${outvar} "${_out}" PARENT_SCOPE )
endfunction( od_json_format )

function( od_json_indent fnm )
    if( NOT EXISTS "${fnm}" )
	message( FATAL_ERROR "od_json_indent: file not found: ${fnm}" )
	return()
    endif()

    file( READ "${fnm}" _json )
    string( STRIP "${_json}" _json )
    string( JSON _root_type ERROR_VARIABLE _e TYPE "${_json}" )
    if( _e )
	message( FATAL_ERROR "od_json_indent: invalid JSON in ${fnm}: ${_e}" )
	return()
    endif()

    od_json_compact( "${_json}" _compact )
    od_json_format( "${_compact}" _indented )

    file( WRITE "${fnm}" "${_indented}\n" )

endfunction( od_json_indent )

function( OD_CREATE_VSCODE_LAUNCHER )
    ...
    if ( NOT DEFINED VSCODE_DEBUGGER_TYPE )
	if ( CMAKE_GENERATOR MATCHES "Visual Studio.*" )
	    set( VSCODE_DEBUGGER_TYPE "cppvsdbg" )
	elseif( APPLE )
	    set( VSCODE_DEBUGGER_TYPE "lldb" )
	elseif( UNIX AND NOT APPLE )
	    set( VSCODE_DEBUGGER_TYPE "cppdbg" )
	endif()
    endif()

    if ( NOT VSCODE_DEBUGGER_TYPE STREQUAL "cppvsdbg" AND
	 NOT VSCODE_DEBUGGER_TYPE STREQUAL "cppdbg" AND
	 NOT VSCODE_DEBUGGER_TYPE STREQUAL "lldb" )
	message( WARNING "Invalid VSCode debugger type: ${VSCODE_DEBUGGER}. Skipping VSCode launcher creation." )
	return()
    endif()

    set( VSCODE_MIMODE "" )
    set( VSCODE_MIPATH "" )
    set( VSCODE_SETUPCOMMANDS "[]" )
    if ( VSCODE_DEBUGGER_TYPE STREQUAL "cppdbg" )
	if ( VSCODE_DEBUGGER STREQUAL "lldb" OR (APPLE AND NOT DEFINED VSCODE_DEBUGGER) )
	    set( VSCODE_MIMODE "lldb" )
	    find_program( CMAKE_LLDB lldb
		PATHS
		/usr/bin
	    ...
	    if ( EXISTS "${CMAKE_LLDB}" )
		set( VSCODE_MIPATH "${CMAKE_LLDB}" )
	    else()
		message( WARNING "Cannot find the lldb debugger" )
	    endif()
	elseif( VSCODE_DEBUGGER STREQUAL "gdb" OR (UNIX AND NOT APPLE AND NOT DEFINED VSCODE_DEBUGGER) )
	    ...
	    if ( EXISTS "${CMAKE_GDB}" )
		set( VSCODE_MIPATH "${CMAKE_GDB}" )
		set( VSCODE_SETUPCOMMANDS "[{ \"description\": \"Enable pretty-printing\", \"text\": \"-enable-pretty-printing\", \"ignoreFailures\": true }]" )
	    else()
		message( WARNING "Cannot find the gdb debugger" )
	    endif()
	endif()
    endif()
	...
	set( _pathsep ";" )
	set( _pathstr "PATH" )
	set( VSCODE_CPP_PROPERTIES_NAME "Windows (CMake)" )
	set( VSCODE_CPP_PROPERTIES_INTELLISENSE_MODE "windows-msvc-x64" )
	set( VSCODE_ODMAIN_APP "od_main_console" )
    else()
	set( _pathsep ":" )
	if ( APPLE )
	    set( _pathstr "DYLD_LIBRARY_PATH" )
	    set( VSCODE_CPP_PROPERTIES_NAME "macOS (CMake)" )
	    set( VSCODE_CPP_PROPERTIES_INTELLISENSE_MODE "mac-clang-x64" )
	else()
	    set( _pathstr "LD_LIBRARY_PATH" )
	    set( VSCODE_CPP_PROPERTIES_NAME "Linux (CMake)" )
	    set( VSCODE_CPP_PROPERTIES_INTELLISENSE_MODE "linux-gcc-x64" )
	endif()
	set( VSCODE_ODMAIN_APP "od_main" )
    endif()
    set( VSCODE_ODPROCESSATTRIB_APP "od_process_attrib" )
    set( VSCODE_ODPROCESSVOLUME_APP "od_process_volume" )

    if ( APPLE )
	set( VSCODE_DEBUG_PATHENVVAL "${CMAKE_BINARY_DIR}/Contents/MacOS/Debug" )
	set( VSCODE_RELEASE_PATHENVVAL "${CMAKE_BINARY_DIR}/Contents/MacOS" )
	set( ODVSCODE_DEBUG_PATHENVVAL "${OD_BINARY_BASEDIR}/Contents/MacOS/Debug" )
	set( ODVSCODE_RELEASE_PATHENVVAL "${OD_BINARY_BASEDIR}/Contents/MacOS" )
    else()
	set( VSCODE_DEBUG_PATHENVVAL "${CMAKE_BINARY_DIR}/bin/${OD_PLFSUBDIR}/Debug" )
	set( VSCODE_RELEASE_PATHENVVAL "${CMAKE_BINARY_DIR}/bin/${OD_PLFSUBDIR}/Release" )
	set( ODVSCODE_DEBUG_PATHENVVAL "${OD_BINARY_BASEDIR}/bin/${OD_PLFSUBDIR}/Debug" )
	set( ODVSCODE_RELEASE_PATHENVVAL "${OD_BINARY_BASEDIR}/bin/${OD_PLFSUBDIR}/Release" )
    endif()

    if ( NOT "${CMAKE_BINARY_DIR}" STREQUAL "${OD_BINARY_BASEDIR}" )
	set( VSCODE_DEBUG_PATHENVVAL "${VSCODE_DEBUG_PATHENVVAL}${_pathsep}${ODVSCODE_DEBUG_PATHENVVAL}" )
	set( VSCODE_RELEASE_PATHENVVAL "${VSCODE_RELEASE_PATHENVVAL}${_pathsep}${ODVSCODE_RELEASE_PATHENVVAL}" )
    endif()
    set( VSCODE_DEBUG_PATHENVVAL "${VSCODE_DEBUG_PATHENVVAL}${_pathsep}\${env:${_pathstr}}" )
    set( VSCODE_RELEASE_PATHENVVAL "${VSCODE_RELEASE_PATHENVVAL}${_pathsep}\${env:${_pathstr}}" )
    cmake_path( NORMAL_PATH OD_BINARY_BASEDIR OUTPUT_VARIABLE NORMALIZED_OD_BINARY_BASEDIR )
    cmake_path( NORMAL_PATH CMAKE_BINARY_DIR OUTPUT_VARIABLE NORMALIZED_CMAKE_BINARY_DIR )
    cmake_path( NORMAL_PATH ODVSCODE_DEBUG_PATHENVVAL OUTPUT_VARIABLE NORMALIZED_ODVSCODE_DEBUG_PATHENVVAL )
    cmake_path( NORMAL_PATH ODVSCODE_RELEASE_PATHENVVAL OUTPUT_VARIABLE NORMALIZED_ODVSCODE_RELEASE_PATHENVVAL )
    cmake_path( NORMAL_PATH OD_TESTDATA_DIR OUTPUT_VARIABLE NORMALIZED_OD_TESTDATA_DIR )

    set( VSCODE_DEBUGENV "" )
    set( VSCODE_RELENV "" )
    if ( VSCODE_DEBUGGER_TYPE STREQUAL "lldb" )
	set( VSCODE_DEBUGENV "{}" )
	set( VSCODE_RELENV "{}" )
	string( JSON VSCODE_DEBUGENV SET "${VSCODE_DEBUGENV}" "${_pathstr}" "\"${VSCODE_DEBUG_PATHENVVAL}\"" )
	string( JSON VSCODE_RELENV SET "${VSCODE_RELENV}" "${_pathstr}" "\"${VSCODE_RELEASE_PATHENVVAL}\"" )
	if ( NOT "${CMAKE_BINARY_DIR}" STREQUAL "${OD_BINARY_BASEDIR}" )
	    string( JSON VSCODE_DEBUGENV SET "${VSCODE_DEBUGENV}" "DTECT_APPL" "\"${NORMALIZED_OD_BINARY_BASEDIR}\"" )
	    string( JSON VSCODE_DEBUGENV SET "${VSCODE_DEBUGENV}" "OD_USER_PLUGIN_DIR" "\"${NORMALIZED_CMAKE_BINARY_DIR}\"" )
	    string( JSON VSCODE_RELENV SET "${VSCODE_RELENV}" "DTECT_APPL" "\"${NORMALIZED_OD_BINARY_BASEDIR}\"" )
	    string( JSON VSCODE_RELENV SET "${VSCODE_RELENV}" "OD_USER_PLUGIN_DIR" "\"${NORMALIZED_CMAKE_BINARY_DIR}\"" )
	endif()
    else()
	set( VSCODE_DEBUGENV "[]" )
	set( VSCODE_RELENV "[]" )
	set( VSCODE_PATHENV "{}")
	string( JSON VSCODE_PATHENV SET "${VSCODE_PATHENV}" "name" "\"${_pathstr}\"" )
	if ( "${CMAKE_BINARY_DIR}" STREQUAL "${OD_BINARY_BASEDIR}" )
	    string( JSON VSCODE_PATHENV SET "${VSCODE_PATHENV}" "value" "\"${VSCODE_DEBUG_PATHENVVAL}\"" )
	    string( JSON VSCODE_DEBUGENV SET "${VSCODE_DEBUGENV}" 0 "${VSCODE_PATHENV}" )
	    string( JSON VSCODE_PATHENV SET "${VSCODE_PATHENV}" "value" "\"${VSCODE_RELEASE_PATHENVVAL}\"" )
	    string( JSON VSCODE_RELENV SET "${VSCODE_RELENV}" 0 "${VSCODE_PATHENV}" )
	else()
	    set( VSCODE_JSENV1 "{}" )
	    string( JSON VSCODE_JSENV1 SET "${VSCODE_JSENV1}" "name" "\"DTECT_APPL\"" )
	    string( JSON VSCODE_JSENV1 SET "${VSCODE_JSENV1}" "value" "\"${NORMALIZED_OD_BINARY_BASEDIR}\"" )

	    set( VSCODE_JSENV2 "{}" )
	    string( JSON VSCODE_JSENV2 SET "${VSCODE_JSENV2}" "name" "\"OD_USER_PLUGIN_DIR\"" )
	    string( JSON VSCODE_JSENV2 SET "${VSCODE_JSENV2}" "value" "\"${NORMALIZED_CMAKE_BINARY_DIR}\"" )
	    ...
	    string( JSON VSCODE_DEBUGENV SET "${VSCODE_DEBUGENV}" 1 "${VSCODE_JSENV1}" )
	    string( JSON VSCODE_DEBUGENV SET "${VSCODE_DEBUGENV}" 2 "${VSCODE_JSENV2}" )
	    string( JSON VSCODE_RELENV SET "${VSCODE_RELENV}" 1 "${VSCODE_JSENV1}" )
	    string( JSON VSCODE_RELENV SET "${VSCODE_RELENV}" 2 "${VSCODE_JSENV2}" )
	endif()
    endif()
    get_template_filepath( "../launcher-templates/vscode_launch_${VSCODE_DEBUGGER_TYPE}" VSCODE_LAUNCH_TEMPLATE )
    if ( NOT EXISTS "${VSCODE_DIR}/launch.json" )
	configure_file( "${VSCODE_LAUNCH_TEMPLATE}" "${VSCODE_DIR}/launch.json" @ONLY )
	od_json_indent( "${VSCODE_DIR}/launch.json" )
    endif()
    if ( (CMAKE_GENERATOR MATCHES "Ninja" OR CMAKE_GENERATOR MATCHES "Makefile") AND
         NOT EXISTS "${VSCODE_DIR}/c_cpp_properties.json" )
	get_template_filepath( "../launcher-templates/c_cpp_properties.json" VSCODE_CPP_PROPERTIES_TEMPLATE )
    	configure_file( "${VSCODE_CPP_PROPERTIES_TEMPLATE}" "${VSCODE_DIR}/c_cpp_properties.json" @ONLY )
    endif()
    set( VSCODE_DIR "${VSCODE_DIR}" PARENT_SCOPE )

endfunction( OD_CREATE_VSCODE_LAUNCHER )
