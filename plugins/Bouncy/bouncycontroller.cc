...
void BouncyController::simulateCB( CallBacker* )
{
    ...
    {
	int randx =Stats::uniformRandGen().getIndex(int(maxpos_.x - minpos_.x));
	int randy =Stats::uniformRandGen().getIndex(int(maxpos_.y - minpos_.y));
	int randz = Stats::uniformRandGen().getIndex(int((zStop_-zStart_)*100));
