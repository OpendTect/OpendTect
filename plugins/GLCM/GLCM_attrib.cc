...
const char* GLCM_attrib::attribTypeStr( int attrType )
{
	...
	if ( attrType==mAttributeF12 ) return "Information Measures of Correlation f12";
	if ( attrType==mAttributeF13 ) return "Information Measures of Correlation f13";
...
{
//	BufferString attribute = desc.getValParam( attributeStr() )->getStringValue();
//	BufferString direction = desc.getValParam( directionStr() )->getStringValue();
	...
	, usegreylevels_(0)
{
	...
	gate_.start = -2 * samprange_;
	gate_.stop = 2 * samprange_;
...
int GLCM_attrib::computeGreyLevel( float trcval) const
{
			...
			if ( trcval < 0 )
				trcval=0;
			if ( trcval > (usegreylevels_-1) )
				trcval=usegreylevels_-1;
...
{
	struct Node *LinkedList = new Node;
		...
		float outval = 0;
				...
				for ( int isamp=sampgate_.start; isamp<=(sampgate_.stop-1); isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-1); posidx++ )
					{
						if ( (posidx+1) % ( 2*stepout_.inl()+1 ) != 0 || posidx == 0)
						{
							...
							if ( dosteer_ )
							{
								const int steeridx = posandsteeridx_.steerindexes_[posidx+1];
								shift = getInputValue( *steerdata_, steeridx, idx, z0 );
							}
							const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
							const int valJ = getInputValue( *inpdata_[posidx+1], dataidx_, idx+isamp+shift+1, z0 );
							greyI = computeGreyLevel( valI );
							greyJ = computeGreyLevel( valJ );
			...
			if ( direction_ == 2 || direction_ == 13 || direction_ == 14 || direction_ == 16 )
			{
				for ( int isamp=sampgate_.start; isamp<=(sampgate_.stop-1); ++isamp)
				{
					...
					{
						const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp, z0 );
						const int valJ = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+1, z0 );
						greyI = computeGreyLevel( valI );
							greyJ = computeGreyLevel( valJ );
			...
			if ( direction_ == 4 || direction_ == 15 || direction_ == 16 )
			{
				for ( int isamp=sampgate_.start; isamp<=sampgate_.stop; isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-(2*stepout_.inl()+1) ); posidx++ )
					{
						if ( (posidx+1) % ( 2*stepout_.inl()+1 ) != 0 || posidx == 0)
						{
							float shift = 0;
							if ( dosteer_ )
							{
								const int steeridx = posandsteeridx_.steerindexes_[posidx+stepout_.inl()*2+2];
								shift = getInputValue( *steerdata_, steeridx, idx, z0 );
							}
							const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
							const int valJ = getInputValue( *inpdata_[posidx+stepout_.inl()*2+2], dataidx_, idx+isamp+shift, z0 );
							greyI = computeGreyLevel( valI );
							greyJ = computeGreyLevel( valJ );
				...
				for ( int isamp=sampgate_.start; isamp<=(sampgate_.stop-1); isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-(2*stepout_.inl()+1) ); posidx++ )
					{
						if ( (posidx+1) % ( 2*stepout_.inl()+1 ) != 0 || posidx == 0)
						{
							...
							const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
							const int valJ = getInputValue( *inpdata_[posidx+stepout_.inl()*2+2], dataidx_, idx+isamp+shift+1, z0 );
							greyI = computeGreyLevel( valI );
							greyJ = computeGreyLevel( valJ );
			...
			if ( direction_ == 7 || direction_ == 14 || direction_ == 15 || direction_ == 16 )
			{
				for ( int isamp=sampgate_.start; isamp<=sampgate_.stop; isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-(stepout_.inl()*2+1)); posidx++ )
					{
						float shift = 0;
						if ( dosteer_ )
						{
							const int steeridx = posandsteeridx_.steerindexes_[posidx+(stepout_.inl()*2+1)];
							shift = getInputValue( *steerdata_, steeridx, idx, z0 );
						}
						const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
						const int valJ = getInputValue( *inpdata_[posidx+(stepout_.inl()*2+1)], dataidx_, idx+isamp+shift, z0 );
						greyI = computeGreyLevel( valI );
						greyJ = computeGreyLevel( valJ );
			...
			if ( direction_ == 8 || direction_ == 14 || direction_ == 16 )
			{
				for ( int isamp=sampgate_.start; isamp<=(sampgate_.stop-1); isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-(stepout_.inl()*2+1)); posidx++ )
					{
						...
						const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
						const int valJ = getInputValue( *inpdata_[posidx+(stepout_.inl()*2+1)], dataidx_, idx+isamp+shift+1, z0 );
						greyI = computeGreyLevel( valI );
						greyJ = computeGreyLevel( valJ );
				...
				for ( int isamp=sampgate_.start; isamp<=sampgate_.stop; isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-(2*stepout_.inl()+1) ); posidx++ )
					{
						if ( (posidx) % ( 2*stepout_.inl()+1 ) != 0 )
						{
							...
							if ( dosteer_ )
							{
								const int steeridx = posandsteeridx_.steerindexes_[posidx+stepout_.inl()*2];
								shift = getInputValue( *steerdata_, steeridx, idx, z0 );
							}
							const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
							const int valJ = getInputValue( *inpdata_[posidx+stepout_.inl()*2], dataidx_, idx+isamp+shift, z0 );
							greyI = computeGreyLevel( valI );
							greyJ = computeGreyLevel( valJ );
				...
				for ( int isamp=sampgate_.start; isamp<=(sampgate_.stop-1); isamp++)
				{
					for ( int posidx=0; posidx<(inpdata_.size()-(2*stepout_.inl()+1) ); posidx++ )
					{
						if ( (posidx) % ( 2*stepout_.inl()+1 ) != 0 )
						{
							...
							const int valI = getInputValue( *inpdata_[posidx], dataidx_, idx+isamp+shift, z0 );
							const int valJ = getInputValue( *inpdata_[posidx+stepout_.inl()*2], dataidx_, idx+isamp+shift+1, z0 );
							greyI = computeGreyLevel( valI );
							greyJ = computeGreyLevel( valJ );
			...
			//case 13:
			//	{
			//		trcval = computeMaximumCorrelationCoefficient(LinkedList, elements);
			//		break;
			//	}
			case 13:
				{
...
std::pair<double, double>  GLCM_attrib::computeMu(struct Node *LinkedList, int elements) const
{
	double MuX = 0.0;
	double MuY = 0.0;
...
std::pair<double, double>  GLCM_attrib::computeSigma(struct Node *LinkedList, int elements, double MuX, double MuY) const
{
	double SigmaX = 0.0;
	double SigmaY =0.0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeMean(struct Node *LinkedList, int elements) const
{
	double Mean = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeMuXminusY(struct Node *LinkedList, int elements) const
{
	double MuXminusY=0;
...
double GLCM_attrib::computeMXplusY(struct Node *LinkedList, int elements, int N) const
{
	double MXplusY=0;
	Node *values=LinkedList;
	...
	while ( values )
	{
		if ( values->nodeI + values->nodeJ == N )
		{
			double mval = values->numbercoocurrence / static_cast<double>(elements);
			MXplusY += mval;
		}
...
double GLCM_attrib::computeMXminusY(struct Node *LinkedList, int elements, int N) const
{
	double MXminusY=0;
	Node *values=LinkedList;
	...
	while ( values )
	{
		if ( (values->nodeI - values->nodeJ == N ) || (values->nodeI - values->nodeJ == -N ) )
		{
			double mval = values->numbercoocurrence / static_cast<double>(elements);
			MXminusY += mval;
		}
...
double GLCM_attrib::computeMxI(struct Node *LinkedList, int elements, int i) const
{
	double MxI = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeMyJ(struct Node *LinkedList, int elements, int j) const
{
	double MyJ = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeHXY1(struct Node *LinkedList, int elements) const
{
	double HXY1 = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeHXY2(struct Node *LinkedList, int elements) const
{
	double HXY2 = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeHX(struct Node *LinkedList, int elements) const
{
	double HX = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeHY(struct Node *LinkedList, int elements) const
{
	double HY = 0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeEnergy(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeContrast(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeCorrelation(struct Node *LinkedList, int elements) const
{
	...
	while ( values )
	{
		...
		int j = values->nodeJ;
		double  mval=values->numbercoocurrence / static_cast<double>(elements);
		trcval += (static_cast<double>(i+1)*static_cast<double>(j+1)*mval-Mu.first*Mu.second)/(Sigma.first*Sigma.second);
		values=values->next;
	}
	return trcval;
}

double GLCM_attrib::computeVariance(struct Node *LinkedList, int elements) const
{
...
double GLCM_attrib::computeInverseDifferenceMoment(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
	...
	while ( values )
	{
		...
		int j = values->nodeJ;
		double mval=values->numbercoocurrence / static_cast<double>(elements);
		trcval += mval/(1+static_cast<double>(i-j+2)*static_cast<double>(i-j+2));
		values=values->next;
	}
	return trcval;
}

double GLCM_attrib::computeSumAverage(struct Node *LinkedList, int elements) const
{
	double trcval=0;
...
double GLCM_attrib::computeSumVariance(struct Node *LinkedList, int elements) const
{
	double trcval=0;
...
double GLCM_attrib::computeSumEntropy(struct Node *LinkedList, int elements) const
{
	double trcval=0;
...
double GLCM_attrib::computeEntropy(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeDifferenceVariance(struct Node *LinkedList, int elements) const
{
	double trcval=0;
...
double GLCM_attrib::computeDifferenceEntropy(struct Node *LinkedList, int elements) const
{
	double trcval=0;
...
double GLCM_attrib::computeF12(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	double MaxHX_HY=0;
...
double GLCM_attrib::computeF13(struct Node *LinkedList, int elements) const
{
	double trcval=0;
...
double GLCM_attrib::computeHomogeneity(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeSumMean(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeMaximumProbability(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeClusterTendency(struct Node *LinkedList, int elements) const
{
	std::pair<double, double> Mu(0.0, 0.0);
	Mu = computeMu(LinkedList, elements);
...
double GLCM_attrib::computeClusterShade(struct Node *LinkedList, int elements) const
{
	std::pair<double, double> Mu(0.0, 0.0);
	Mu = computeMu(LinkedList, elements);
...
double GLCM_attrib::computeClusterProminence(struct Node *LinkedList, int elements) const
{
	std::pair<double, double> Mu(0.0, 0.0);
	Mu = computeMu(LinkedList, elements);
...
double GLCM_attrib::computeDissimilarity(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeDifferenceMean(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeAutocorrelation(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
...
double GLCM_attrib::computeInertia(struct Node *LinkedList, int elements) const
{
	double trcval=0;
	Node *values=LinkedList;
	...
	trcval = trcval/(((usegreylevels_+1)*(usegreylevels_+1)-1)*((usegreylevels_+1)*(usegreylevels_+1)-1));
	return trcval;
}
