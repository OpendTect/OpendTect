...
-*/
static const char* rcsID = "$Id: cmddriverbasics.cc,v 1.25 2012/07/10 13:05:57 cvskris Exp $";
...
const char* windowTitle( const uiMainWin* uimw, int aliasnr )
{
    if ( uimw==ODMainWin() )
    {
	if ( aliasnr == 0 )
...
const uiMainWin* WindowStack::topWin() const
{
    const_cast<WindowStack*>(this)->synchronize();
    return winstack_.size() ? winstack_[0] : 0;
}
