...
-*/
static const char* rcsID = "$Id: cmddriver.cc,v 1.105 2012/03/26 13:57:34 cvsjaap Exp $";
...
void ModalStatus::get()
{
    ModalStatusActivator modalstatusinspector( *this );
    CallBack cb = mCB( &modalstatusinspector, ModalStatusActivator, actCB );
    ODMainWin()->activateInGUIThread( cb ); 
}
...
void CmdDriver::prepareForAbort()
{
	...
	if ( !activityStopped(false,false) )
	{
	    ...
	    ODMainWin()->activateInGUIThread( cb, false );
	    nrtimesinactive = 0;
	}
	else
	    nrtimesinactive++;
...
void CmdDriver::mkThread( CallBacker* )
{
    ...
    ODMainWin()->activatedone.notify( mCB(this,CmdDriver,activateDone) );
    ...
    winstack_.moveFrameToTop( ODMainWin() );
	...
	if ( pause_ && !openqdialog_ )
	{
		...
		ODMainWin()->activateInGUIThread( icb, false );
	    }
    ...
    ODMainWin()->activatedone.remove( mCB(this,CmdDriver,activateDone) );
    getNrActiveCmdDrivers( -1 );
...
void CmdDriver::setWait( float time, bool regular )
{
    if ( regular )
    {
	const float regtime = time<0.0 ? 0.0 : time;
...
{ return windowTitle( (openqdialog_ ? 0 : curWin()), aliasnr ); }
...
void CmdDriver::finishActivate( bool busywait )
{
    mBusyWaitUntil( abort_ || !busywait || activityStopped() );
...
void CmdDriver::forceQtToCatchUp() const
{
    // Hack changing the display state of one (dummy) object in ODMainWin
    // back and forth, while busy waiting for those tasks to be finished.
    // Afterwards, any previous task is considered to be finished as well.
    
    ObjectSet<const uiObject> objsfound; 
    ObjectFinder objfinder( *ODMainWin() );
    objfinder.findNodes( ObjectFinder::CurWinTopGrp, &objsfound ); 
    uiObject* dummy;
    for ( int idx=0; idx<objsfound.size(); idx++ )
    {
	dummy = const_cast<uiObject*>( objsfound[idx] );
    ...
    // TODO: Find a decent solution to force Qt to catch up.
}
...
bool CmdDriver::verifyWinAssert( const char* newassertion )
{
    ...
    SearchKey(winstr,winassertcs_).getMatchingWindows( windowlist );
    ...
    if ( newassertion )
	SearchKey(winstr,winassertcs_).getMatchingWindows( windowlist, wcm_ );
...
bool CmdDriver::verifyWinState( const char* newwinstr, WinStateType newwinstate)
{
    ...
    mParDisambiguator( "window name", winstr, selnr );
    ObjectSet<uiMainWin> windowlist;
    mSearchKey(winstr).getMatchingWindows( windowlist );
    mKeepSelection( windowlist, selnr );
    ...
    {
	mSearchKey(winstr).getMatchingWindows( windowlist, wcm_ );
...
void CmdDriver::interact( const InteractSpec* ispec )
{
    ...
    ODMainWin()->activateInGUIThread( mCB(this,CmdDriver,interactCB), false );
}
