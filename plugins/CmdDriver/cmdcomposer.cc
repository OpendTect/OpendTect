...
-*/
static const char* rcsID = "$Id: cmdcomposer.cc,v 1.19 2011/03/02 14:27:00 cvsjaap Exp $";
				   ...
				   const char* extrakey, CmdRecorder& cmdrec )
{
    ...
    mGetComposerClass( Tree, uiListView,		caller, 0, cmdrec );
    mGetComposerClass( CanvasMenu, uiGraphicsView,	caller, 0, cmdrec );
...
bool CmdComposer::traceSrcWin( CmdRecEvent& ev ) const
{
	...
	if ( !strcmp(eventlist_[idx]->idstr_, ev.idstr_) )
	{
	    ...
	    ev.interrupt_ = eventlist_[idx]->interrupt_;
...
void CmdComposer::shrinkEventList( int firstnr, int lastnr )
{
    ...
    if ( firstidx<0 || firstidx>=sz || lastidx<0 || lastidx>=sz )
	return;

    for ( int idx=sz-1; idx>=0; idx-- )
    {
	if ( firstidx>lastidx && idx<firstidx && idx>lastidx )
	    continue;
	if ( firstidx<=lastidx && (idx<firstidx || idx>lastidx) )
	    continue;
