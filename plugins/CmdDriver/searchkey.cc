...
-*/
static const char* rcsID = "$Id: searchkey.cc,v 1.4 2012/03/26 13:57:34 cvsjaap Exp $";
...
void SearchKey::getMatchingWindows( ObjectSet<uiMainWin>& windowlist,
       				    WildcardManager* wcm ) const
{
	...
	if ( !isCmdDriverWindow(windowlist[idx]) )
	{
	    ...
	    {
		const char* alias = windowTitle( windowlist[idx], aliasnr );
		if ( !alias )
		{
