...
-*/
static const char* rcsID = "$Id: treecommands.cc,v 1.14 2011/07/27 09:22:49 cvsjaap Exp $";
			   ...
			   bool colbydefault=false )
{
    ...
    const char* parnexxt;
    const int num = strtol( parnext, const_cast<char**>(&parnexxt), 0 );
...
#define mListViewTrigger( notifier ) \
{ \
...
#define mListViewTrigger2( notifier1, notifier2 ) \
{ \
    mListViewTrigger( notifier1 ); \
...
#define mHandleLeftRightClick() \
	...
	mListViewTrigger2( leftButtonClicked, mouseButtonClicked ) \
    else if ( actclicktags_.isPresent("Right") ) \
	mListViewTrigger2( rightButtonClicked, mouseButtonClicked ) \
    else \
	mListViewTrigger( mouseButtonClicked );
...
void TreeActivator::actCB( CallBacker* cb )
{
	...
	else
	{
	    ...
	    if ( actclicktags_.isPresent("Left") )
		mListViewTrigger2( leftButtonPressed, mouseButtonPressed )
	    else if ( actclicktags_.isPresent("Right") )
		mListViewTrigger2( rightButtonPressed, mouseButtonPressed )
	    else
		mListViewTrigger( mouseButtonPressed );

	    mHandleLeftRightClick();
	    if ( actclicktags_.isPresent("Double") )
	    {
		mListViewTrigger( doubleClicked );
		mHandleLeftRightClick();
	    }
	}
    }
...
bool TreeExpandCmd::act( const char* parstr )
{
    ...
    mDynamicCastGet( const uiListView*, uilview, objsfound[0] );
    mParTreeSelPre( uilview, treepath, nodesfound, pathstr, false );
    uiListViewItem* curnode = const_cast<uiListViewItem*>( nodesfound[0] );
...
#define mGetListView( uilview, retval ) \
\
