...
-*/
static const char* rcsID = "$Id: menubutcommands.cc,v 1.31 2011/08/03 09:30:32 cvsjaap Exp $";
...
bool MenuCmd::act( const char* parstr )
{
    ...
    mFindMenuItem( "Survey`Exit", *ODMainWin()->menuBar(), surveyexit );
...
bool CloseCmd::actCloseCurWin( const char* parstr )
{
	...
	{
	    if ( windowlist[idx] != ODMainWin() )
		mActivate( Close, Activator(*windowlist[idx]) );
	}
	mActivateNoWait( Close, Activator(*ODMainWin()) );
	return true;
    }
