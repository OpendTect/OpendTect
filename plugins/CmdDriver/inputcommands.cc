...
-*/
static const char* rcsID = "$Id: inputcommands.cc,v 1.24 2012/07/10 13:05:57 cvskris Exp $";
...
bool SliderCmd::act( const char* parstr )
{
    ...
    char* parnexxt;
    const float finalfrac = strtod( parnext, &parnexxt) / 100; 
...
bool WheelCmd::act( const char* parstr )
{
    ...
    char* parnexxt;
    const float angle = strtod( parnext, &parnexxt) * M_PI / 180; 
    if ( parnexxt == parnext )
    {
...
bool SpinCmdComposer::accept( const CmdRecEvent& ev )
{
    ...
    const char* msgnext;
    const float oldval = strtod( ev.msg_, const_cast<char**>(&msgnext) );
    mSkipBlanks( msgnext );
...
bool WheelCmdComposer::accept( const CmdRecEvent& ev )
{
	...
	const float angle = 180*(newvalue-oldvalue_) / M_PI;
	insertWindowCaseExec( ev );
