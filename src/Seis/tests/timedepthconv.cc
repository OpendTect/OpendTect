...
{
    ConstRefMan<SimpleTimeDepthTransform> stretcher =
					new SimpleT2DTransform( mid );
    ...
    const ZSampling zrgfrom = stretcher->getZInterval_( true, false );
    const ZSampling zrgto = stretcher->getZInterval_( false, false );
...
{
    const double v0 =
	    LinearVelTransformNew::velUnit()->getUserValueFromSI( 3000. );
    const double k = 0.1;
    ConstRefMan<LinearVelTransformNew> stretcher =
					new LinearT2DTransformNew( v0, k );
...
static bool testLinearD2TTransform()
{
    ...
    const double k = 0.1;
    ConstRefMan<LinearVelTransformNew> stretcher =
					new LinearD2TTransformNew( v0, k );
...
{
    ConstRefMan<VelocityStretcherNew> stretcher =
					new Time2DepthStretcherNew( mid );
...
{
    ConstRefMan<VelocityStretcherNew> stretcher =
					new Depth2TimeStretcherNew( mid );
...
static bool testVelocityModelScanner( const MultiID& mid )
{
    ...
    VelocityModelScannerNew scanner( *velobj, desc );
