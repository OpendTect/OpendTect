...
HiddenParam<SeisIOSimple,const Survey::Geometry2D*> hp_geom2d(nullptr);
...
SeisIOSimple::SeisIOSimple( const Data& d, bool imp )
    ...
    , zistm_(SI().zIsTime())
{
    const Survey::Geometry2D* geom2d = nullptr;
    const bool is2d = Seis::is2D( data_.geom_ );
    if ( is2d )
    {
	mDynamicCast(const Survey::Geometry2D*,geom2d,
		     Survey::GM().getGeometry(data_.geomid_));
	if ( geom2d && geom2d->isEmpty() )
	    geom2d = nullptr;
    }

    hp_geom2d.setParam( this, geom2d );
