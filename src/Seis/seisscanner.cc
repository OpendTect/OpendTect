...
void SeisScanner::report( StringPairSet& report ) const
{
    ...
    report.add( "Z step", sampling_.step );
    report.add( "Z start in file", sampling_.start );
    report.add( "Z stop in file", zRange().stop );

    report.add( "Number of samples in file", (int)nrsamples_ );
    if ( nonnullsamplerg_.start != 0 )
	report.add( "First non-zero sample", nonnullsamplerg_.start + 1 );
    if ( nonnullsamplerg_.stop != nrsamples_-1 )
	report.add( "Last non-zero sample", nonnullsamplerg_.stop + 1 );
	...
	getSurvInfo(cs,crd);
	report.add( "Z.start", cs.zsamp_.start );
	report.add( "Z.stop", cs.zsamp_.stop );
	report.add( "Z.step", cs.zsamp_.step );
    }

    if ( !mIsUdf(valrg_.start) )
    {
	report.add( StringPairSet::sKeyH1(), "Data values" );
	report.add( "Minimum value", valrg_.start );
	report.add( "Maximum value", valrg_.stop );
