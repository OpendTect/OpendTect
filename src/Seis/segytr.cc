...
bool SEGYSeisTrcTranslator::writeTapeHeader()
{
	...
	fileopts_.thdef_.linename = curlinekey;
    ...
    filepars_.fmt_ = binhead_.format();
    binhead_.setEntryVal( SEGY::BinHeader::EntryLino(), pinfo.nr );
    mDefineStaticLocalObject( int, jobid, = 0 );
    binhead_.setEntryVal( SEGY::BinHeader::EntryJobID(), ++jobid );
    binhead_.setNrSamples( outnrsamples );
    binhead_.setSampleRate( outsd.step, mInDepth );
