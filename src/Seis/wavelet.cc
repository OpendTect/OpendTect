...
HiddenParam<Wavelet,ValueSeriesInterpolator<float>*> intpol_(0);

ValueSeriesInterpolator<float>* Wavelet::getIntPol() const
{
    return intpol_.getParam( this );
}
...
Wavelet& Wavelet::operator =( const Wavelet& wv )
{
    ...
    delete intpol_.getParam(this); intpol_.removeParam(this);
    if ( wv.getIntPol() )
	intpol_.setParam( this,
	   new ValueSeriesInterpolator<float>( *wv.getIntPol() ) );
...
{
    delete [] samps_;
    delete intpol_.getParam(this);
    intpol_.removeParam(this);
}
...
const ValueSeriesInterpolator<float>& Wavelet::interpolator() const
{
	...
	= const_cast<ValueSeriesInterpolator<float>&>( intpol_.getParam(this)
				    ? *intpol_.getParam(this) : *defintpol );
    ret.maxidx_ = size() - 1;
    return ret;
}
