...
void SEGY::FileReadOpts::getReport( StringPairSet& report, bool isrev0 ) const
{
	...
	{
	    report.add( "First Trace number", trcnrdef_.start );
	    report.add( "Trace number increment", trcnrdef_.step );
	}
    }
    else
    {
	...
	if ( psdef_ == UsrDef )
	{
	    report.add( "First Offset", offsdef_.start );
	    report.add( "Offset increment", offsdef_.step );
	}
