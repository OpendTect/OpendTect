...
static bool doImport( std::ostream& strm, IOPar& iop, bool is2d )
{
    IOPar* outpar = iop.subselect( sKey::Output() );
    if ( !outpar || outpar->isEmpty() )
	{ strm << "Batch parameters 'Ouput' empty" << std::endl; return false; }

    SEGY::FileSpec fs; fs.usePar( iop );
    IOObj* inioobj = fs.getIOObj( true );
    if ( !inioobj )
	{ strm << "Input file spec is not OK" << std::endl; return false; }
    IOObj* outioobj = IOM().get( outpar->find(sKey::ID()) );
    if ( !outioobj )
	{ strm << "Output object spec is not OK" << std::endl; return false; }
    ...
    return stp->execute( &strm );
}


static bool doScan( std::ostream& strm, IOPar& iop, bool isps, bool is2d )
{
    MultiID mid;
    iop.get( sKey::Output(), mid );
    if ( mid.isEmpty() )
    {
	strm << "Parameter file lacks key '" << sKey::Output() << std::endl;
	return false;
    }
