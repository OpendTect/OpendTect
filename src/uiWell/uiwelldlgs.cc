...
bool uiWellTrackDlg::updNow()
{
	...
	if ( idx > 0 && mIsUdf(dahval) )
	{
        ...
        track_.addPoint( newc, mCast(float,newc.z_), dahval );
    }
...
void uiWellTrackDlg::updatePos( bool isx )
{
    ...
    if ( mIsUdf(surfacepos) && !track_.isEmpty() )
        surfacepos = isx ? track_.pos(0).x_ : track_.pos(0).y_;
...
void uiD2TModelDlg::dtpointChangedCB( CallBacker* )
{
    ...
    if ( row>mdvals_.size()-1 )
    {
	mdvals_.setSize( row+1 );
	tvals_.setSize( row+1 );
    }
...
void uiD2TModelDlg::dtpointRemovedCB( CallBacker* )
{
    ...
    d2t->remove( idah-1 );
    mdvals_.removeSingle( idah-1 );
    tvals_.removeSingle( idah-1 );
...
void uiD2TModelDlg::selectionDeletedCB( CallBacker* )
{
	...
	d2t->remove( idah );
	mdvals_.removeSingle( idah );
	tvals_.removeSingle( idah );
...
bool uiD2TModelDlg::updateDtpoint( int row, float oldval )
{
	...
	d2t->remove( dahidx );
	mdvals_.removeSingle( dahidx );
	tvals_.removeSingle( dahidx );
    }
	...
	{
	    mdvals_.insert( idz, dah );
	    tvals_.insert( idz, twt );
	}
    }
...
void uiD2TModelDlg::getModel( Well::D2TModel& d2t )
{
	...
	const float dah = mdvals_[irow];
	const float twt = tvals_[irow];
	d2t.add( dah, twt );
    }
}
