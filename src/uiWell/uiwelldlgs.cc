...
HiddenParam<uiD2TModelDlg,uiCheckBox*> timefld_( 0 );
        ...
        , unitfld_(0)
{
    ...
    uiCheckBox* timefld = new uiCheckBox( this, " Time is TWT" );
    timefld->setChecked( true );
    timefld->activated.notify( mCB(this,uiD2TModelDlg,fillTable) );
    timefld->attach( rightAlignedBelow, tbl_ );
    timefld_.setParam( this, timefld );
    ...
    unitfld_->attach( rightAlignedBelow, timefld );
...
BufferStringSet uiD2TModelDlg::getColLabels() const
{
    ...
    if ( timefld_.getParam(this) )
	timeisoneway = !timefld_.getParam(this)->isChecked();
...
bool uiD2TModelDlg::updateDtpointDepth( int row )
{
    ...
    const float twtfac = timefld_.getParam(this)->isChecked() ? 2000.f : 1000.f;
