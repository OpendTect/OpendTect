	...
	, origgl_(d.info().groundelev)
{
    ...
    uiButton* readbut = !writable_ ? 0
		: new uiPushButton( iobutgrp, "Import",
...
void uiWellTrackDlg::fillSetFields( CallBacker* )
{
    ...
    kbelevfld_->setTitleText( uiString("%1 %2 ")
...
bool uiWellTrackDlg::updNow( CallBacker* )
{
	...
	{
	    uiString msg = uiString( "Found undefined values in row %1. "
...
void uiWellTrackDlg::updateKbElev( CallBacker* )
{
    ...
    {
	uiMSG().error(
	     BufferString("Please enter a valid ",Well::Info::sKeykbelev()) );
...
bool uiWellTrackDlg::acceptOK( CallBacker* )
{
    ...
    if ( dahchg )
    {
	uiString msg = "You have changed at least one MD value.\nMarkers%1"
			  " are based on the old MD values.\n. "
			  "They may therefore become invalid.\n\nContinue?";
	if ( SI().zIsTime() )
	    msg.arg(", logs, T/D and checkshot models");
	else
	    msg.arg(" and logs");
...
void uiWellTrackDlg::exportCB( CallBacker* )
{
    ...
    if ( !strm.isOK() )
    {
	uiMSG().error( uiString( "Cannot open '%1' for write")
				 .arg(fdlg.fileName()) );
	return;
    }
	...
	, replvelfld_(0)
{
	...
	{
	    uiButton* updbut = new uiPushButton( this, "Update display",
	    ...
	    uiPushButton* setbut = new uiPushButton( this, "Set",
    ...
    new uiPushButton( iobutgrp, "Export",
...
{
    const bool zinfeet = unitfld_ ? unitfld_->isChecked() : false;
    ...
    curlbl.add( UnitOfMeasure::surveyDefTimeUnitAnnot(true,true) );
    lbls.add( curlbl );
...
#define mConvertVal(val,toscreen) ( \
	toscreen ? \
 getConvertedValue( mDataUom(true)->userValue(val),mDataUom(true), \
	    getDisplayUnit(unitfld_) ) : \
 mDataUom(true)->internalValue( \
	     getConvertedValue(val,getDisplayUnit(unitfld_),mDataUom(true)) ) )
...
void uiD2TModelDlg::fillReplVel( CallBacker* )
{
    ...
    BufferString lbl( Well::Info::sKeyreplvel(), " ",
		      getVelUnitString(unitfld_->isChecked(),true) );
    if ( unitfld_->isChecked() ) lbl.addSpace();
...
bool uiD2TModelDlg::updateDtpointDepth( int row )
{
    ...
    {
	uiString errmsg = tracksz<2 ? "Invalid track"
    ...
    uiString errmsg = uiString("The entered %1");
    if ( !zrange.includes(inval,true) )
    {
	errmsg.arg(
	uiString("%1 value %2 is outside of track range\n[%3, %4]%5 %6")
	      .arg(lbl )
	      .arg( mConvertVal(inval,true) )
	      .arg( mConvertVal(zrange.start,true) )
	      .arg( mConvertVal(zrange.stop,true) )
	      .arg(getDistUnitString(unitfld_->isChecked(),true)).arg(lbl));
    ...
    {
	errmsg.arg(uiString("%1 is not between the depths of the previous and "
...
bool uiD2TModelDlg::updateDtpointTime( int row )
{
    ...
    if ( inval < 0.f )
    {
	uiString errmsg = "Negative travel-times are not allowed"
    ...
    {
	uiString errmsg = "The entered time is not between the times "
...
bool acceptOK( CallBacker* )
{
    ...
    {
	const char* emsg = d2tgrp->errMsg();
	if ( emsg && *emsg )
	    uiMSG().error( emsg );

	return false;
    }
    const char* wmsg = d2tgrp->warnMsg();
    if ( wmsg && *wmsg )
	uiMSG().warning( wmsg );
...
void uiD2TModelDlg::expData( CallBacker* )
{
    ...
    if ( !strm.isOK() )
    {
	uiMSG().error( uiString("Cannot open '%1' for write").arg(fnm) );
	return;
    }
...
void uiD2TModelDlg::updReplVelNow( CallBacker* )
{
    ...
    {
	uiMSG().error( BufferString("Please enter a valid ",
...
void uiD2TModelDlg::correctD2TModelIfInvalid()
{
    ...
    BufferString errmsg, warnmsg;
    if ( !d2t->ensureValid(wd_,0,0,&errmsg,&warnmsg) || d2t->size() < 2 )
    {
	BufferString msg( "Invalid model detected" );
	if ( !errmsg.isEmpty() )
	    msg.addNewLine().add( errmsg );

	if ( !warnmsg.isEmpty() )
	    msg.addNewLine().add( warnmsg );

	msg.addNewLine().add( "But could not autocorrect the current model" );
	uiMSG().warning( msg );
	if ( *d2t != *orgd2t_ )
	    needrestore = true;
    }
