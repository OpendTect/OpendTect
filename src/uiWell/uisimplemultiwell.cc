...
-*/
static const char* rcsID mUnusedVar = "$Id: uisimplemultiwell.cc,v 1.13 2012-05-22 16:41:57 cvsnanne Exp $";
...
class BulkWellAscIO : public Table::AscIO
{
public:
BulkWellAscIO( const Table::FormatDesc& fd, std::istream& strm )
    : Table::AscIO(fd)
    , strm_(strm)
{}
...
static Table::FormatDesc* getDesc()
{
    ...
    fd->bodyinfos_ += new Table::TargetInfo( "MD", FloatInpSpec(), Table::Optional );
...
uiBulkWellImport::uiBulkWellImport( uiParent* p )
    : uiDialog(p,uiDialog::Setup("Bulk Well Import",mNoDlgTitle,mTODOHelpID))
    , fd_(BulkWellAscIO::getDesc())
{
    inpfld_ = new uiFileInput( this, "Input file", uiFileInput::Setup()
		      .withexamine(true).examstyle(uiFileInput::Setup::Table) );
...
uiBulkWellImport::~uiBulkWellImport()
{
    delete fd_;
    deepErase( wells_ );
}
...
bool uiBulkWellImport::acceptOK( CallBacker* )
{
    const BufferString fnm( inpfld_->fileName() );
    if ( fnm.isEmpty() )
	mErrRet( "Please enter the input file name" )
    ...
    BulkWellAscIO aio( *fd_, *sd.istrm );
    ...
    if ( !errors.isEmpty() )
	uiMSG().errorWithDetails( errors, "Could not import all wells (see details)" );
...
bool uiBulkLogImport::acceptOK( CallBacker* )
{
	...
	PtrMan<IOObj> ioobj = IOM().getLocal( info.wellnm );
	if ( !ioobj )
	{
	...
	lasimp.setData( wd );
	errmsg = lasimp.getLogs( filenms.get(idx), info );
	if ( !errmsg.isEmpty() )
	    errors.add( BufferString(fnm,": ",errmsg) );
    }
    ...
    uiMSG().errorWithDetails( errors, "Could not import all LAS files (See details)" );
    return true;
}
