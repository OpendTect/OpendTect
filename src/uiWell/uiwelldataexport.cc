    ...
    , wellpartserver_(wps)
{
    ...
    mAttachCB( postFinalise(), uiWellExportFacility::inputChngCB );
}
...
{
    FilePath outfp( fp );
    const FixedString basenm = basenmfld_->text();
    BufferString filenm;
    if ( basenm.isEmpty() )
	filenm = type;
    else
...
bool uiWellExportFacility::exportD2TModel( const char* fp, bool ischksht )
{
    ...
    const float groundevel = mConvertVal(wd_->info().groundelev);
	...
	*strm << UnitOfMeasure::surveyDefTimeUnit()->userValue(d2t->t(idx)) /
	...
	vint = replvellayer.includes( -1.f * track.getPos(dah).z, true ) &&
	    !mIsUdf(info.replvel) ?
	    info.replvel : mCast(float,d2t->getVelocityForDah(dah,track));
