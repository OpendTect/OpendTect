...
bool StratAmpCalc::init( const IOPar& pars )
{
    ...
    if ( pars.get(sKeyAmplitudeOption(),ampopt) &&
	 !parseAmplitudeOption(ampopt,stattyp_,summode_) )
	return false;
	...
	if ( outfold_ )
	{
	    ...
	    if ( dataidxfold >= 0 )
		dataidxsfold_ += dataidxfold;

	    posidfold_.setObjectID( addtohor.id() );
	}
    }

    if ( dataidxs_.size() != selcomps_.size() )
	return false;

    if ( outfold_ && dataidxs_.size()!=dataidxsfold_.size() )
	return false;
...
int StratAmpCalc::nextStep()
{
    ...
    for ( int idx=0; idx<dataidxs_.size(); idx++ )
    {
	...
	for ( int sidx=0; sidx<sampintv.nrSteps()+1; sidx++ )
	{
	    ...
	    const float val = trc->getValue( zval, usesstored_ ? selcomps_[idx]
							       : idx );
	    if ( mIsUdf(val) )
		continue;

	    if ( stattyp_ == Stats::Sum && summode_ != SumMode::All )
	    {
		if ( summode_ == SumMode::Positive && val <= 0 )
		    continue;
		if ( summode_ == SumMode::Negative && val >= 0 )
		    continue;
	    }
	...
	addtohor.auxdata.setAuxDataVal( dataidxs_[idx], posid_, outval );
	if ( outfold_ )
	{
	    posidfold_.setSubID( subid );
	    addtohor.auxdata.setAuxDataVal( dataidxsfold_[idx], posidfold_,
