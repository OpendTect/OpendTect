				 ...
				 Attrib::DescID ignoreid )
{
    ...
    {
	ConstRefMan<Desc> desc = attrdata_.attribid_.isValid()
			? attrdata_.attrSet().getDesc( attrdata_.attribid_ )
			: nullptr;
	if ( desc )
	{
	    ...
	    {
		StringPair sp( desc->userRef() );
		storcur = attrinf_->ioobjnms_.indexOf( sp.first() );
