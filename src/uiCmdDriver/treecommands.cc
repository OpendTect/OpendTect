...
\
	...
	{ \
		...
		const BufferString itemtxt = row<0 \
			? uilview->getColumnText(col).getString() \
...
\
    bool pathcol = false; \
    BufferString tagstr; \
    const char* mUnusedVar parnext = getNextNonBlanks(parstr,tagstr.getCStr());\
\
...
bool CurTreeColCmd::act( const char* parstr )
{
    ...
    mParForm( answer, form, uilview->getColumnText(curcolidx).getString(),
	      curcolidx+1 );
...
bool GetTreeColCmd::act( const char* parstr )
{
    ...
    mParColSelPre( "column",uilview,nodesfound,itemstr,itemnr,columns,true );
    mParForm( answer, form, uilview->getColumnText(columns[0]).getString(),
	      columns[0]+1 );
    mParIdentPost( identname, answer, parnext );
    return true;
}
...
bool TreeCmdComposer::accept( const CmdRecEvent& ev )
{
	...
	getNextNonBlanks( ev.msg_, notifiername.getCStr() );
