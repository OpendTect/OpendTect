...
{
    return SI().binID2Coord().transform( BinID(rc) );
}
...
bool BinIDSurface::isKnotDefined( const RowCol& rc ) const
{
    const float zval = getZ( rc );
    return !mIsUdf(zval);
}


float BinIDSurface::getZ( const BinID& bid ) const
{
