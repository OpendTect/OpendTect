...
uiDataPointSetCrossPlotter::Setup::Setup()
    : noedit_(false)
    , markerstyle_(OD::MarkerStyle2D::Square)
    , xstyle_(OD::LineStyle::Solid,1,Color::Black())
    , ystyle_(OD::LineStyle::Solid,1,Color::stdDrawColor(0))
    , y2style_(OD::LineStyle::Dot,1,Color::stdDrawColor(1))
...
{
    info.appendPhrase(tr("Selection Group Name : %1").arg(name()));
	...
	range = selarea.getValueRange(false);
	info.appendPhrase(tr("%1 (range) :%2, %3").arg(selarea.yaxisnm_)
              .arg(range.start).arg(range.stop));
