    ...
    , mathexprstring1_(plotter_.userdefy2str_)
{
    ...
    const Color y1col = plotter_.getUserDefLineColor( true );
	...
	const Color y2col = plotter_.getUserDefLineColor( false );
...
bool acceptOK()
{
	...
	plotter_.setUserDefLineColor( y2colorbut_->color(), false );
	plotter_.setup().showy2userdefpolyline_ =
    ...
    plotter_.setUserDefLineColor( y1colorbut_->color(), true );
    drawPolyLines();
    return true;
}
    ...
    , plotter_(p->plotter())
{
    ...
    nrbinsxfld_ = new uiGenInput( this, tr("Nr bins X-axis"),
		IntInpSpec(plotter_.nrBins(true),1) );
    ...
    nrbinsyfld_ = new uiGenInput( this, tr("Nr bins Y-axis"),
		IntInpSpec(plotter_.nrBins(false),1) );
...
bool acceptOK()
{
    plotter_.setNrBins( nrbinsxfld_->getIntValue(), nrbinsyfld_->getIntValue());
