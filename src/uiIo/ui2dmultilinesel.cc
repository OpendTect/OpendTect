...
const char* ui2DMultiLineSel::getLineSet()  // TODO return multi linesets
{
    ...
    return S2DPOS().getLineSet( geomids_[0].lsID() );
}
...
BufferStringSet ui2DMultiLineSel::getSelLines() const
{
    ...
    S2DPOS().setCurLineSet( geomids_[0].lsID() );
    for ( int idx=0; idx<geomids_.size(); idx++ )
    {
	sellines.add( S2DPOS().getLineName(geomids_[idx].lineID()) );
	if ( (idx+1) != geomids_.size() )
	    if ( geomids_[idx].lsID() != geomids_[idx+1].lsID() )
		S2DPOS().setCurLineSet( geomids_[idx+1].lsID() );
    }
...
void ui2DMultiLineSelDlg::fillUIForMultiLineSet()
{
    ...
    {
	BufferString lsnm;
	lsnm = S2DPOS().getLineSet( selgeomids_[0].lsID() );
	lsnmsfld_->setItemChecked( lsnm.buf(), true );
	previouslsnm_ = lsnm;
	for ( int idx=1; idx<selgeomids_.size(); idx++ )
	{
	    if ( selgeomids_[idx].lsID() != selgeomids_[idx-1].lsID() )
	    {
		lsnm = S2DPOS().getLineSet( selgeomids_[idx].lsID() );
		lsnmsfld_->setItemChecked( lsnm.buf(), true );
	    }
...
void ui2DMultiLineSelDlg::fillUIForSingleLineSet()
{
    ...
    if ( selgeomids_.size() > 0 )
    {
	lsnmfld_->setCurrentItem( S2DPOS().getLineSet(selgeomids_[0].lsID()));
	previouslsnm_ = S2DPOS().getLineSet(selgeomids_[0].lsID());
    }
    else
    {
	...
	{ lsnmfld_->setReadOnly( true ); return; }
}
...
void ui2DMultiLineSelDlg::fillUIForMultiLine()
{
    ...
    {
	BufferString lnm;
	int test = selgeomids_[0].lsID(); ////debug
	S2DPOS().getLines( lnms, selgeomids_[0].lsID() );
	lnmsfld_->addItems( lnms );
	S2DPOS().setCurLineSet( S2DPOS().getLineSet(selgeomids_[0].lsID()) );
	lnm = S2DPOS().getLineName( selgeomids_[0].lineID() );
	...
	for ( int idx=1; idx<selgeomids_.size(); idx++ )
	{
	    if ( selgeomids_[idx].lsID() != selgeomids_[idx-1].lsID() )
		break;
	    lnm = S2DPOS().getLineName( selgeomids_[idx].lineID() );
	    lnmsfld_->setItemChecked( lnm, true );
	}
    }
    else
    {
	S2DPOS().getLines( lnms, lineinfo_[0]->geomid_.lsID() );
	lnmsfld_->addItems( lnms );
...
void ui2DMultiLineSelDlg::fillUIForSingleLine()
{
	...
	S2DPOS().setCurLineSet( selgeomids_[0].lsID() );
	lnm = S2DPOS().getLineName( selgeomids_[0].lineID() );
...
void ui2DMultiLineSelDlg::multiLineSetSel( CallBacker* )
{
    ...
    {
	if ( selgeomids_[idx].lsID() == gid.lsID() )
	    lnmsfld_->setItemChecked(
		    S2DPOS().getLineName(selgeomids_[idx].lineID()), true );
    }
