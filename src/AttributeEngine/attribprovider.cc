...
static const char* rcsID = "$Id: attribprovider.cc,v 1.108 2008-09-22 13:06:42 cvskris Exp $";
...
od_int64 totalNr() const { return nrsamples_; }
...
const DataHolder* Provider::getData( const BinID& relpos, int idi )
{
	...
	if ( !outdata->series(idx) )
	{
		...
		valptr = new ArrayValueSeries<float,float>( ptr, true );
	    }
	    else
