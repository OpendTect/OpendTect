...
bool BatchProgram::go( std::ostream &strm )
{
    ...
    {
	std::cerr << "Failed while comparing values in different surveys\n";
	return false;
    }
...
bool testAnglesForDifferentSurveys()
{
	...
	fp.setRange( false, zrange );
	computer->setOutputSampling( fp );
	computer->setTraceID( BinID(426,950) );
	if ( !computer->isOK() )
	{
	    std::cerr << survnames[idx];
	    std::cerr << " : Angle computer is not OK.\n";
	...
	{
	    std::cerr << survnames[idx];
	    std::cerr << " : Angle computer computed wrong values\n";
	    return false;
	}
    }
