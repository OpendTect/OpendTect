...
static HiddenParam<StratSynth::DataMgr,Notifier<StratSynth::DataMgr>*>
				ssdatamgrhpmgr_(nullptr);
...
{
    ssdatamgrhpmgr_.setParam( this, new Notifier<StratSynth::DataMgr>(this) );
...
{
    detachAllNotifiers();
    ssdatamgrhpmgr_.removeAndDeleteParam( this );
    setEmpty();
    deepErase( lmdatasets_ );
}


Notifier<StratSynth::DataMgr>& StratSynth::DataMgr::elasticModelChanged()
{
    return *ssdatamgrhpmgr_.getParam( this );
}
...
{
    bool changed = false;
    return ensureElasticModels( lmsidx, reqtype, changed, taskrun );
}
...
bool doPrepare( int nrthreads ) override
{
	...
	for ( int idx=0; idx<nrthreads; idx++ )
	{
	    ...
	    filters_ += filter;
	    if ( sgp_.filtertype_()=="LowPass" && sgp_.freqselrg_().size()==2 )
		filter->setLowPass( sgp_.freqselrg_()[0],
				    sgp_.freqselrg_()[1] );
	    else if ( sgp_.filtertype_()=="HighPass" &&
						sgp_.freqselrg_().size()==2 )
		filter->setHighPass( sgp_.freqselrg_()[0],
				     sgp_.freqselrg_()[1] );
	    else if ( sgp_.filtertype_()=="BandPass" &&
						sgp_.freqselrg_().size()==4 )
		filter->setBandPass( sgp_.freqselrg_()[0],
				     sgp_.freqselrg_()[1],
				     sgp_.freqselrg_()[2],
				     sgp_.freqselrg_()[3] );
	    else
