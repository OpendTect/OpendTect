    ...
    : uiEditDirectFileDataDlg( p, obj )
{
	...
	lbl->attach( alignedBelow, selFld() );
	return;
    }

    fillFileTable();
    filetable_->attach( stretchedBelow, selFld() );
}
...
void uiSeisDirectFileDataDlg::updateFileTable( int rowidx )
{
    ...
    const BufferString seldir = selFld()->text();
...
void uiSeisDirectFileDataDlg::fileSelCB( CallBacker* cb )
{
    ...
    FilePath fp( selFld()->text(), newfnm );
...
bool uiSeisDirectFileDataDlg::acceptOK( CallBacker* )
{
    ...
    if ( !filetable_ )
    {
	const BufferString newfnm = selFld()->text();
	if ( newfnm.isEmpty() )
