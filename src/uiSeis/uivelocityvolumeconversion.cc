...
bool Vel::uiBatchVolumeConversion::fillPar()
{
    ...
    OD::VelocityType type = outputdesc.getType();
    if ( OD::VelocityTypeDef().parse(outputtype,type) )
    {
	outputdesc.setType( type );
	Worker::setUnit( Worker::getUnit(inputveldesc), outputdesc );
