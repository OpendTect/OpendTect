...
uiMergeSeisBatch::uiMergeSeisBatch( uiParent* p,
		       ...
		       mODHelpKey(mMergeSeisHelpID)))
{
    ...
    addbut_ = new uiToolButton( selectgrp, "rightarrow", uiStrings::sAdd(),
				mCB(this,uiMergeSeisBatch,addCubeCB) );
    removebut_ = new uiToolButton( selectgrp, "leftarrow",
				   uiStrings::sRemove(),
				   mCB(this,uiMergeSeisBatch,removeCubeCB) );
    ...
    moveupbut_ = new uiToolButton( ordergrp, "uparrow", uiStrings::sMoveUp(),
				   mCB(this,uiMergeSeisBatch,moveCubeCB) );
    ...
    Batch::JobSpec js( "od_merge_seis" );
    ...
    mAttachCB( availablefld_->selectionChanged,
	       uiMergeSeisBatch::selectionChangedCB );
    mAttachCB( availablefld_->itemChosen,
	       uiMergeSeisBatch::selectionChangedCB );
    mAttachCB( availablefld_->doubleClicked,
	       uiMergeSeisBatch::addCubeCB );
    mAttachCB( selectedfld_->selectionChanged,
	       uiMergeSeisBatch::selectionChangedCB );
    mAttachCB( selectedfld_->doubleClicked,
	       uiMergeSeisBatch::removeCubeCB );
    updateButtonSensitivity();
}


uiMergeSeisBatch::~uiMergeSeisBatch()
{
    detachAllNotifiers();
}


void uiMergeSeisBatch::fillAvailableCubes( const TypeSet<MultiID>* selids )
{
    availableids_.erase();
    BufferStringSet nms;
...
void uiMergeSeisBatch::insertAvailable( const MultiID& mid )
{
...
void uiMergeSeisBatch::refreshAvailableList()
{
    BufferStringSet nms;
...
void uiMergeSeisBatch::addCubeCB( CallBacker* )
{
    TypeSet<int> idxs;
...
void uiMergeSeisBatch::removeCubeCB( CallBacker* )
{
...
void uiMergeSeisBatch::moveCubeCB( CallBacker* cb )
{
...
void uiMergeSeisBatch::selectionChangedCB( CallBacker* )
{
    updateButtonSensitivity();
}


void uiMergeSeisBatch::updateSelectedList( int curidx )
{
...
void uiMergeSeisBatch::updateButtonSensitivity()
{
...
void uiMergeSeisBatch::updateTransfld()
{
    TrcKeyZSampling tkzs;
    for ( const auto& mid : selectedids_ )
    {
...
bool uiMergeSeisBatch::acceptOK( CallBacker* )
{
    ObjectSet<IOPar> inpars;
    IOPar outpar;
    if ( !getInput(inpars,outpar) )
...
bool uiMergeSeisBatch::getInput( ObjectSet<IOPar>& inpars, IOPar& outpar )
{
    outfld_->reset();
    const IOObj* outioobj = outfld_->ioobj();
    if ( !outioobj )
