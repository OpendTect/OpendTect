...
bool uiTextFile::open( const char* fnm )
{
	...
	Table::Converter cnvrtr( *imphndlr.ptr(), exphndlr, setup_.maxnrlines_);
	cnvrtr.execute();
    }
...
bool uiTextFileDlg::rejectOK( CallBacker* cb )
{
    ...
    if ( !editor_->reLoad() )
	doMsg(
	    tr("Cannot re-load file. Possibly the file no longer exists.") );
...
int uiTextFileDlg::doMsg( const uiString& msg, bool iserr )
{
    ...
    uiMsgMainWinSetter uimsgsetter( this );
    if ( iserr )
	uiMSG().error( msg );
    else
