...
mDefineEnumUtils( uiColTabExport, ExportType, "Export type" )
    { "Other user", "OpendTect *_coltab", "Petrel *.alut", nullptr };

template <>
void EnumDefImpl<uiColTabExport::ExportType>::init()
{
...
uiColTabExport::uiColTabExport( uiParent* p )
    : uiDialog(p,uiDialog::Setup(uiStrings::phrExport(uiStrings::sColorTable()),
				  mNoDlgTitle, mODHelpKey(mColTabExportHelpID)))
    , dirfld_(0)
    , dtectusrfld_(0)
{
    setOkText( uiStrings::sExport() );
    choicefld_ = new uiGenInput( this, tr("Export from"),
				 StringListInpSpec(ExportTypeDef()) );
    choicefld_->valueChanged.notify( mCB(this,uiColTabExport,choiceSel) );
    ...
    dirfld_ = new uiFileInput( this, getLabel(OtherUser),
			       ...
			       .directories(true) );
    dirfld_->setReadOnly();
    dirfld_->valueChanged.notify( mCB(this,uiColTabExport,usrSel) );
    ...
    dtectusrfld_->updateRequested.notify( mCB(this,uiColTabExport,usrSel) );
...
const char* uiColTabExport::getCurrentSelColTab() const
{
    return listfld_->getText();
}


void uiColTabExport::choiceSel( CallBacker* )
{
    const auto imptype = sCast(ExportType,choicefld_->getIntValue());
    switch( imptype )
    {
	case OtherUser:
...
void uiColTabExport::usrSel( CallBacker* )
{
...
void uiColTabExport::getFromSettingsPar( const IOPar& par )
{
    deepErase( seqs_ );
    int nrinvalididx = 0;
    for ( int idx=0; ; idx++ )
...
void uiColTabExport::getFromAlutFiles( const BufferStringSet& filenms )
{
    deepErase( seqs_ );
...
bool uiColTabExport::acceptOK( CallBacker* )
{
    ...
    uiString msg = tr( "Color table successfully exported."
...
void uiColTabExport::showMessage( const uiString& msg )
{
...
uiString uiColTabExport::getLabel( ExportType imptype )
{
    switch( imptype )
    {
