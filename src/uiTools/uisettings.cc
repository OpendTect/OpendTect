    ...
    : terminalRequested(this)
{
    ...
    auto* progargs = new ObjectSet<const BufferStringSet>;
...
{
    BufferStringSet& prognms_ = getProgNames();
    ObjectSet<const BufferStringSet>& progargs_ = getProgArgs();
...
void uiSettingsMgr::doTerminalCmdCB( CallBacker* cb )
{
    ...
    const BufferStringSet& prognms_ = getProgNames();
    BufferString prognm;
...
class MaterialGroup : public uiGroup
{ mODTextTranslationClass(MaterialGroup)
public:
MaterialGroup( uiParent* p, Settings& setts )
...
static HiddenParam<uiVisSettingsGroup,MaterialGroup*> hp_materialgrp( nullptr );
    ...
    , anisotropicpower_(4)
{
    ...
    auto* matgrp = new MaterialGroup( this, setts_ );
    matgrp->attach( alignedBelow, anisotropicpowerfld_ );
    hp_materialgrp.setParam( this, matgrp );
...
{
    hp_materialgrp.removeParam( this );
}
...
bool uiVisSettingsGroup::acceptOK()
{
    ...
    auto* matgrp = hp_materialgrp.getParam( this );
    matgrp->updateSettings( changed_ );
