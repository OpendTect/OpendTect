...
void uiStratLayerModel::lmDispParsChangedCB( CallBacker* )
{
    ...
    if ( !sd ) return;
    sd->dispPars().vdMapper() = lmpropdp.mapper_;
    sd->dispPars().coltab_ = lmpropdp.ctab_;
    sd->dispPars().setOverlap( lmpropdp.overlap_ );
...
void uiStratLayerModel::synthDispParsChangedCB( CallBacker* )
{
    ...
    vddisppars.mapper_ = vdsd->dispPars().vdMapper();
    vddisppars.ctab_ = vdsd->dispPars().coltab_;
    vddisppars.overlap_ = vdsd->dispPars().overlap();
