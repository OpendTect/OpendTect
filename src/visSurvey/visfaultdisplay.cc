...
void FaultDisplay::setDepthAsAttrib( int attrib )
{
	...
	while ( bivs.next(pos) )
	{
		...
		vals[zcol] = zaxistransform_->transformTrc(
					TrcKey(bivs.getBinID(pos)), vals[0] );
	    }
	    else
		vals[zcol] = vals[0];
	}
...
void FaultDisplay::getRandomPosCache( int attrib, DataPointSet& data ) const
{
    ...
    const auto dpid = getDataPackID( attrib );
    auto& dpman = DPM( DataPackMgr::SurfID() );
...
void FaultDisplay::setMarkerStyle( const OD::MarkerStyle3D& mkstyle )
{
    ...
    if ( fault_ )
	fault_->setPreferredMarkerStyle3D( ftmkstyle );
}
