...
int Patch::getNearestRdmLineIdx( const TrcKey& tk ) const
{
	...
	ParamLine2 linesegment( firstcrd, secondcrd );
	const double closestval = linesegment.closestPoint(poscrd);
	const Coord closestcrd = linesegment.getPoint( closestval );
