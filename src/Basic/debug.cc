...
void CrashDumper::sendDump( const char* filename )
{
    ...
    const FilePath symboldir( GetBinPlfDir(), "symbols" );

#ifdef __win__
    const FilePath dumphandler(GetBinPlfDir(), "minidump_stackwalk.exe");
#else
    const FilePath dumphandler( GetBinPlfDir(), "minidump_stackwalk" );
#endif
	...
	args += BufferString( " \"",
		FilePath(GetBinPlfDir(),sendappl_).fullPath(), "\"" );
