...
static
    HiddenParam<OD::PythonAccess,CNotifier<OD::PythonAccess,const uiRetVal&>* >
					odpythaenvverifnotifmgr_(nullptr );
...
{
    odpythaenvverifnotifmgr_.setParam( this,
		    new CNotifier<OD::PythonAccess,const uiRetVal&>( this ) );
...
OD::PythonAccess::~PythonAccess()
{
    detachAllNotifiers();
    delete activatefp_;
    odpythaenvverifnotifmgr_.removeAndDeleteParam( this );
}


CNotifier<OD::PythonAccess,const uiRetVal&>& OD::PythonAccess::envVerified()
{
    return *odpythaenvverifnotifmgr_.getParam( this );
}
...
{
    return isUsable( force, (BufferString*)nullptr );
...
{
    uiRetVal ret; BufferString stdoutmsg, stderrmsg;
    return isUsable_( force, ret, &stdoutmsg, &stderrmsg );
}
...
{
    uiRetVal ret;
    return isEnvUsable( pythonenvfp, envnm, ret );
}
...
{
    pErrMsg( "Obsolete, should not be called" );
    uiRetVal uirv;
    return execute( cmd, uirv, wait4finish );
}
				...
				int* pid, uiString* errmsg ) const
{
    pErrMsg( "Obsolete, should not be called" );
    uiRetVal ret;
    const bool res = execute( cmd, pars, ret, pid );
    if ( !res && errmsg )
	*errmsg = ret.messages().cat();
				      ...
				      bool /* wait4finish */ ) const
{
    pErrMsg( "Obsolete, should not be called" );
    BufferString stdoutstr; uiRetVal ret;
    return executeScript( scriptstr, stdoutstr, ret );
}
				      ...
				      bool /* wait4finish */ ) const
{
    pErrMsg( "Obsolete, should not be called" );
    BufferString stdoutstr; uiRetVal ret;
    return executeScript( scriptstrs, stdoutstr, ret );
}
...
bool OD::PythonAccess::executeScript( const BufferStringSet& scriptstrs,
				      ...
				      BufferString* stderrstr ) const
{
    return executeScript( BufferString(scriptstrs.cat(";")), wait4finish, ret,
			  stdoutstr, stderrstr );
}


BufferString OD::PythonAccess::lastOutput( bool, uiString* ) const
{
    pErrMsg( "Obsolete, should not be called" );
...
bool OD::PythonAccess::isModuleUsable( const char* nm, uiRetVal& ret ) const
{
   BufferStringSet importscript;
    ...
    return executeScript( importscript, true, ret );
}
...
{
    return hasModule( moduleinfos_, modname, true, minversion );
}
...
{
    return hasModule( moduleinfos, modname, true, minversion );
}
...
{
    uiRetVal ret;
    return openTerminal( cmdstr, ret, args, workingdirstr );
}
...
uiRetVal pythonRemoveDir( const char* path, bool waitforfin )
{
    ...
    if ( PythA().isUsable(false,(BufferString*)nullptr).isOK() )
	res = PythA().executeScript( removeDirScript(path), waitforfin, ret );
    else
