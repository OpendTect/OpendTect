...
static const char* rcsID = "$Id: survinfo.cc,v 1.44 2003-10-28 18:08:58 nanne Exp $";
...
{
    return StepInterval<int>( range().start.inl, range().stop.inl,
	    		      getStep(true,work) );
}
...
{
    return StepInterval<int>( range().start.crl, range().stop.crl,
	    		      getStep(false,work) );
}
