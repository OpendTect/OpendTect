...
bool FSStoFault3DConverter::readSection()
{
...
bool FSStoFault3DConverter::readSectionForImport()
{
    mDynamicCast(const Geometry::FaultStickSet*,curfssg_,fss_.geometryElement())
    if ( !curfssg_ )
	return false;
...
bool FSStoFault3DConverter::writeSection() const
{
    if ( sticks_.isEmpty() )
	return false;
	...
	for ( int crdidx=1; crdidx<stick->crds_.size(); crdidx++ )
	{
	    ...
	    fault3d_.geometry().insertKnot( rc.toInt64(),
