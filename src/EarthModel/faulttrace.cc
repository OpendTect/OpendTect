			   ...
			   BinID& bidimg, float& zimg, bool posdir ) const
{
    ...
    float z1 = posdir ? ztop.start_ : ztop.stop_;
    float z2 = posdir ? zbot.start_ : zbot.stop_;

    const float frac = mIsEqual(z1,z2,1e-6) ? 0 : ( z - z1 ) / ( z2 - z1 );
    z1 = posdir ? ztop.stop_ : ztop.start_;
    z2 = posdir ? zbot.stop_ : zbot.start_;
    zimg = z1 + frac * ( z2 - z1 );
	...
	TypeSet<Coord>& intersections, bool firstonly, bool allowextend ) const
{
    ...
    for ( int idx=1; ; idx++ )
    {
        const int trcnr = trcrg.start_-trcrg.step_*idx;
        if ( trcnr < hortrcrg.start_ ) break;

	const BinID curbid( isinl_ ? nr_ : trcnr, isinl_ ? trcnr : nr_ );
	const float curz = use2d ?
                               (float) hor2d->getPos( geomid, trcnr ).z_ :
                               (float) hor.getPos( curbid.toInt64() ).z_;
    ...
    for ( int idx=1; ; idx++ )
    {
        const int trcnr = trcrg.stop_+trcrg.step_*idx;
        if ( trcnr > hortrcrg.stop_ ) break;
	...
	if ( !mIsUdf(prevz) )
	{
            const int nrsteps = (trcnr-prevtrc)/hortrcrg.step_;
    ...
    const BinID extbid0( isinl_ ? nr_ : trcrange_.start_,
                         isinl_ ? trcrange_.start_ : nr_ );
    const BinID extbid1( isinl_ ? nr_:trcrange_.stop_,
                         isinl_ ? trcrange_.stop_:nr_ );
				   ...
				   float stopz, TypeSet<Coord>& coords ) const
{
    ...
    {
	const double y0 =
                mMIN(tracesegs_[idx].start_.y_,tracesegs_[idx].stop_.y_);
	const double y1 =
                mMAX(tracesegs_[idx].start_.y_,tracesegs_[idx].stop_.y_);

	const double x0 =
                mMIN(tracesegs_[idx].start_.x_,tracesegs_[idx].stop_.x_);
	const double x1 =
                mMAX(tracesegs_[idx].start_.x_,tracesegs_[idx].stop_.x_);
        if ( (pos0.y_>=y0 && pos0.y_<=y1) || (starttrc>=x0 && starttrc<=x1) )
	{
        ...
        if ( (pos1.y_>=y0 && pos1.y_<=y1) || (stoptrc>=x0 && stoptrc<=x1) )
	{
	...
	const int segidx = startsegidx==-1  ? stopsegidx : startsegidx;
        if ( (tracesegs_[segidx].start_.y_>=minz &&
              tracesegs_[segidx].start_.y_<=maxz) ||
             (tracesegs_[segidx].start_.x_>=mintrc &&
              tracesegs_[segidx].start_.x_<=maxtrc) )
...
bool FaultTrace::getIntersectionTraces( float zval, TypeSet<int>& trcs ) const
{
    ...
    for ( int idx=0; idx<tracesegs_.size(); idx++ )
    {
        const double z0 = tracesegs_[idx].start_.y_;
        const double z1 = tracesegs_[idx].stop_.y_;
	if ( (z<z0 && z<z1) || (z>z0 && z>z1) )
	    continue;

        const double t0 = tracesegs_[idx].start_.x_;
        const double t1 = tracesegs_[idx].stop_.x_;
...
bool FaultTrace::getIntersectionZs( int trc, TypeSet<float>& zs ) const
{
    ...
    for ( int idx=0; idx<tracesegs_.size(); idx++ )
    {
        const int trc0 = mNINT32(tracesegs_[idx].start_.x_);
        const int trc1 = mNINT32(tracesegs_[idx].stop_.x_);
	if ( (trc<trc0 && trc<trc1) || (trc>trc0 && trc>trc1) )
	    continue;

        const float z0 = mCast(float,tracesegs_[idx].start_.y_) /
	    SI().showZ2UserFactor();
        const float z1 = mCast(float,tracesegs_[idx].stop_.y_) /
	    SI().showZ2UserFactor();
...
{
    if ( trcrange_.stop_ < flttrc.trcrange_.start_ ||
         trcrange_.start_ > flttrc.trcrange_.stop_ ||
         zrange_.stop_ < flttrc.zrange_.start_ ||
         zrange_.start_ > flttrc.zrange_.stop_ )
	return false;

    for ( int idx=0; idx<flttrc.tracesegs_.size(); idx++ )
    {
        const int trc0 = mNINT32(flttrc.tracesegs_[idx].start_.x_);
	const BinID bid0( isinl_ ? nr_ : trc0, isinl_ ? trc0 : nr_ );
        const int trc1 = mNINT32(flttrc.tracesegs_[idx].stop_.x_);
	const BinID bid1( isinl_ ? nr_ : trc1, isinl_ ? trc1 : nr_ );

        const float z0 = mCast(float,flttrc.tracesegs_[idx].start_.y_) /
	    SI().showZ2UserFactor();
        const float z1 = mCast(float,flttrc.tracesegs_[idx].stop_.y_) /
	    SI().showZ2UserFactor();

	const Coord intsect = getIntersection( bid0, z0, bid1, z1 );
	if ( intsect.isDefined() )
	{
            trace = mCast(int,intsect.x_);
            zval = mCast(float,intsect.y_);
	    return true;
	}
    }
			...
			const BinID& posbid, bool istop ) const
{
    ...
    BinID origin( isinl_ ? nr_ : trcrange_.stop_+10,
                  isinl_ ? trcrange_.stop_+10 : nr_ );
	...
	if ( !intersectn.isDefined() )
        ...
        if ( fabs(intersectn.x_ - (float)trcvar) > 10 )
	    break;
    ...
    bid = posbid;
    prevz = mUdf(float);
    origin = BinID( isinl_ ? nr_ : trcrange_.start_-10,
                    isinl_ ? trcrange_.start_-10 : nr_ );
    for ( int idx=0; idx<1024; idx++,trcvar++ )
    {
	BinIDValueSet::SPos pos = bvs.find( bid );
	if ( !pos.isValid() )
				  ...
				  Interval<float>& zbot ) const
{
    ...
    BinID start( isinl_ ? nr_ : trcrange_.start_ - 10*step,
                 isinl_ ? trcrange_.start_ - 10*step : nr_ );
    ...
    BinID stop( isinl_ ? nr_ : trcrange_.stop_ + 10*step,
                isinl_ ? trcrange_.stop_ + 10*step: nr_ );
    ...
    if ( !tophasisect && !mIsUdf(zrange_.start_) )
        prevstoptopz = stoptopz = zrange_.start_;
    if ( !bothasisect && !mIsUdf(zrange_.stop_) )
        prevstopbotz = stopbotz = zrange_.stop_;
...
float FaultTrace::getZValFor( const BinID& bid ) const
{
    ...
    Coord intersectn = getIntersection( bid, zrg.start_, bid, zrg.stop_ );
...
bool FaultTrace::isOnPosSide( const BinID& bid, float z ) const
{
    ...
    if ( trcnr >= trcrange_.stop_ )
	return true;
    if ( trcnr <= trcrange_.start_ )
	return true;

    const BinID startbid( isinl_ ? nr_ : trcrange_.start_,
                          isinl_ ? trcrange_.start_ : nr_ );
    const BinID stopbid( isinl_ ? nr_ : trcrange_.stop_,
                         isinl_ ? trcrange_.stop_ : nr_ );
    const float zmid = ( zrange_.start_ + zrange_.stop_ ) / 2;
...
void FaultTrace::computeTraceSegments()
{
	...
	if ( is2d )
	{
	    nodepos1.setXY( getTrcNr(previdx),
                            pos1.z_ * SI().zDomain().userFactor() );
	    nodepos2.setXY( getTrcNr(curidx),
                            pos2.z_ * SI().zDomain().userFactor() );
	}
	else
	{
	    Coord posbid1 = SI().binID2Coord().transformBackNoSnap( pos1 );
	    Coord posbid2 = SI().binID2Coord().transformBackNoSnap( pos2 );
            nodepos1.setXY( isinl_ ? posbid1.y_ : posbid1.x_,
                            pos1.z_ * SI().zDomain().userFactor() );
            nodepos2.setXY( isinl_ ? posbid2.y_ : posbid2.x_,
                            pos2.z_ * SI().zDomain().userFactor() );
	}
                                  ...
                                  bool snappositive) const
{
    ...
    int trcnr = mNINT32( intersection.x_ );
    if ( intv )
    {
        const float fidx = intv->getfIndex( intersection.x_ );
			    ...
			    float thresholddist ) const
{
    ...
    BinID startbid( isinl_ ? nr_ : trcrange_.start_,
                    isinl_ ? trcrange_.start_ : nr_ );

    BinID stopbid( isinl_ ? nr_ : trcrange_.stop_,
                   isinl_ ? trcrange_.stop_ : nr_ );
...
void FaultTrace::computeRange()
{
	...
	for ( int idx=0; idx<trcnrs_.size(); idx++ )
        ...
        trcrange_.set( (int) floattrcrg.start_, (int) ceil(floattrcrg.stop_) );

	for ( int idx=0; idx<coords_.size(); idx++ )
            zrange_.include( (float) coords_[idx].z_, false );
    }
    else
    {
	for ( int idx=0; idx<coords_.size(); idx++ )
...
bool FaultTrace::isOK() const
{
    ...
    double prevz = coords_[0].z_;
    for ( int idx=1; idx<coords_.size(); idx++ )
    {
        if ( coords_[idx].z_ < prevz )
	    return false;

        prevz = coords_[idx].z_;
    }
...
bool FaultTraceExtractor3D::extractFaultTrace( int idx )
{
    ...
    Coord3 p0( SI().transform(start), zrg.start_ );
    Coord3 p1( SI().transform(start), zrg.stop_ );
    Coord3 p2( SI().transform(stop), zrg.stop_ );
    Coord3 p3( SI().transform(stop), zrg.start_ );
    TypeSet<Coord3> pts;
					  ...
					  TrcKeySampling& hs ) const
{
    ...
    for ( int idx=rowrg.start_; idx<=rowrg.stop_; idx+=rowrg.step_ )
    {
	const StepInterval<int> colrg = fss->colRange( idx );
        for ( int idy=colrg.start_; idy<=colrg.stop_; idy+=colrg.step_ )
	{
