...
-*/
static const char* rcsID = "$Id: emfaultstickset.cc,v 1.7 2010-01-27 13:48:27 cvsjaap Exp $";
...
const Coord3& FaultStickSetGeometry::getEditPlaneNormal( const SectionID& sid,
							 int sticknr ) const
{
    const Geometry::FaultStickSet* fss = sectionGeometry( sid );
...
void FaultStickSetGeometry::copySelectedSticksTo( FaultStickSetGeometry& tofssg,
						  const SectionID tosid ) const
{
	...
	const Geometry::FaultStickSet* fromfss = sectionGeometry( sid );
	if ( !fromfss )
	    continue;
...
void FaultStickSetGeometry::selectAllSticks( bool select )
{
...
bool FaultStickSetGeometry::removeNextSelStick()
{
...
void FaultStickSetGeometry::removeSelectedSticks()
{ while ( removeNextSelStick() ); }


int FaultStickSetGeometry::nrSelectedSticks() const
{
