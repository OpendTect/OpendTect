...
bool odEMBodyTranslator::implRemove( const IOObj* ioobj ) const
{
    if ( !ioobj || !Translator::implRemove(ioobj) )
	return false;
...
bool odEMBodyTranslator::implRename( const IOObj* ioobj, const char* newnm,
				      const CallBack* cb ) const
{
    if ( !ioobj || !Translator::implRename(ioobj,newnm,cb) )
	return false;
