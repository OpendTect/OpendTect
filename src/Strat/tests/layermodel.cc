...
static bool testSharedMathOnModel( const PropertyRefSelection& prs )
{
    ...
    ConstRefMan<Math::SharedFormula> vsform =
				getSharedFormula( lm, 3, *castagnaform_ );
    ConstRefMan<Math::SharedFormula> aiform =
				getSharedFormula( lm, 4, *aiform_ );
    mRunStandardTest(
	vsform.ptr() == getSharedFormula( lm, 3, *castagnaform_ ).ptr() &&
	aiform.ptr() == getSharedFormula( lm, 4, *aiform_ ).ptr(),
