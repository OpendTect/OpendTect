...
mDefEmptyTranslatorBundle( StratTreeN, od, Mdl, "od", sKeyStratTree,
			   StratTreeNTranslator::sStratTree )

bool StratTreeNTranslator::implRename( const IOObj* ioobj,
...
bool StratTreeNTranslator::implRemove( const IOObj* ioobj, bool ) const
{
    if ( !ioobj )
	return false;
...
bool StratTreeNTranslator::implSetReadOnly( const IOObj* ioobj, bool yn ) const
{
    if ( !ioobj )
	return false;
...
const char* StratTreeNTranslator::associatedFileExt()
{
    return "stratlevels";
}
