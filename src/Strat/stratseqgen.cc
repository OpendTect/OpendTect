...
static HiddenParam<Strat::LayerSequenceGenDesc,float>
					stratlaygendeschpmgr_(mUdf(float));
    ...
    , rt_(rt)
{
    ...
    stratlaygendeschpmgr_.setParam( this, defOverburdenVelocity() );
...
{
    deepErase( *this );
    stratlaygendeschpmgr_.removeParam( this );
}
...
void Strat::LayerSequenceGenDesc::erase()
{
    ...
    startdepth_ = getConvertedValue( -SI().seismicReferenceDatum(),
				    UnitOfMeasure::surveyDefSRDStorageUnit(),
				    UnitOfMeasure::surveyDefSRDUnit() );
    setOverburdenVelocity( defOverburdenVelocity() );
    propsel_.setEmpty();
...
{
    return stratlaygendeschpmgr_.getParam( this );
}


void Strat::LayerSequenceGenDesc::setOverburdenVelocity( float vel )
{
    stratlaygendeschpmgr_.setParam( this, vel );
}
...
bool Strat::LayerSequenceGenDesc::getFrom( od_istream& strm )
{
    ...
    if ( !mIsUdf(abovevel) )
	setOverburdenVelocity( abovevel );
...
bool Strat::LayerSequenceGenDesc::putTo( od_ostream& strm ) const
{
    ...
    setOverburdenVelocity( sKeyVelAbove, overburdenVelocity(), iop );
					    ...
					    float modpos ) const
{
    ...
    ls.setStartDepthOnly( startDepth() );
