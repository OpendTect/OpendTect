...
class uiBulkFltFSSHandlerGroup : public uiGroup
{ mODTextTranslationClass(uiBulkFltFSSHandlerGroup)
public:
    uiBulkFltFSSHandlerGroup( uiParent* p, const char* type )
	...
	mAttachCB( horzdomypefld_->valueChanged,
			    uiBulkFltFSSHandlerGroup::zDomainTypeChg );
	...
	multisurfdepthread_ = new uiMultiSurfaceRead( multigrp, type,
								&depthinfo );
	multisurfdepthread_->display( false );
	multisurftimeread_ = new uiMultiSurfaceRead( multigrp, type,
							    &ZDomain::TWT() );
	multisurftimeread_->display( true );
    }


    ~uiBulkFltFSSHandlerGroup()
    {
	detachAllNotifiers();
    }
    ...
    Notifier<uiBulkFltFSSHandlerGroup> zdomtypupdated;
...
//uiExportFault
static HiddenParam<uiExportFault,uiBulkFltFSSHandlerGroup*>
    ...
    , typ_(typ)
{
    ...
    if ( isbulk_ )
    {
	auto* bulkgrp = new uiBulkFltFSSHandlerGroup( this, typ );
	mAttachCB( bulkgrp->zdomtypupdated,
					uiExportFault::zDomainBulkTypeChg );
	uiexporthorizonhpmgr_.setParam( this, bulkgrp );
	coordfld_->attach( alignedBelow, bulkgrp );
    }
    else
    {
...
void uiExportFault::zDomainBulkTypeChg(CallBacker*)
{
