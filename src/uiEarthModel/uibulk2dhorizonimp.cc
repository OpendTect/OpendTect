...
{
    auto* fd = new Table::FormatDesc( "Bulk 2D Horizon" );
...
static void createDescBody( Table::FormatDesc* fd, const ZDomain::Def& zdef )
{
    ...
    auto* zti = new Table::TargetInfo( nm, FloatInpSpec(),
						    Table::Required );
...
static HiddenParam<uiBulk2DHorizonImport,uiGenInput*> hp_zdomfld(nullptr);
static HiddenParam<uiBulk2DHorizonImport,uiGenInput*> hp_overwritefld(nullptr);
    ...
    , fd_(BulkHorizon2DAscIO::getDesc(SI().zDomain()))
{
    ...
    mAttachCB( zdomselfld->valueChanged, uiBulk2DHorizonImport::zDomainCB );
    hp_zdomfld.setParam( this, zdomselfld );
    ...
    auto* overwritefld = new uiGenInput( this, tr("When horizon exists"),
				BoolInpSpec(true,tr("Skip"),tr("Overwrite")) );
    overwritefld->attach( alignedBelow, udftreatfld_ );
    hp_overwritefld.setParam( this, overwritefld );
}
...
{
    delete fd_;
    hp_zdomfld.removeParam( this );
    hp_overwritefld.removeParam( this );
}
...
void uiBulk2DHorizonImport::zDomainCB( CallBacker* )
{
    ...
    auto* fld = hp_zdomfld.getParam( this );
    const bool istime = fld->getBoolValue();
    BulkHorizon2DAscIO::updateDesc( *fd_,
...
bool uiBulk2DHorizonImport::acceptOK( CallBacker* )
{
    ...
    const bool doskip = hp_overwritefld.getParam(this)->getBoolValue();
    BufferStringSet existinghorizons;
    EM::IOObjInfo::getObjectNames( EM::IOObjInfo::Horizon2D, existinghorizons );
