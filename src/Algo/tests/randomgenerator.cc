...
static bool testBaseInterface()
{
    Stats::RandGen urg;
    Stats::RandomGenerator& gen = urg;
    mRunStandardTest( gen.setSeed(cBaseInterfaceSeed),
		      "RandomGenerator::setSeed" );

    Stats::RandGen expected;
    expected.setSeed( cBaseInterfaceSeed );
    mRunStandardTest( gen.get()==expected.get(), "RandomGenerator::get" );

    gen.clearSeed();
    mRunStandardTest( gen.get()>=0. && gen.get()<1.,
		      "RandomGenerator::get after clearSeed" );
