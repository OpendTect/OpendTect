...
void DataClipper::putData( float val )
{
    if ( subselect_ )
    {
	double rand = Stats::uniformRandGen().get();
...
void DataClipSampler::add( const float* v, od_int64 sz )
{
    ...
    od_int64 randint = Stats::uniformRandGen().getIndex( mUdf(int) );
    for ( int idx=0; idx<nr2add; idx++ )
    {
	od_int64 vidx = Stats::uniformRandGen().getIndexFast( sz, randint );
	doAdd( v[vidx] );
...
{
    if ( Stats::uniformRandGen().getIndex(count_) < maxnrvals_ )
	doAdd( val );
}
...
void DataClipSampler::doAdd( float val )
{
	...
	vals_[ Stats::uniformRandGen().getIndex(maxnrvals_) ] = val;
