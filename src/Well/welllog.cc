...
mDefineInstanceCreatedNotifierAccess(Well::LogInfo);
...
Well::LogInfo::LogInfo( const Well::LogInfo& oth )
    : NamedMonitorable(oth)
{
    dahrg_.setUdf();
    mTriggerInstanceCreatedNotifier();
...
Well::LogInfo::~LogInfo()
{
    sendDelNotif();
}


mImplMonitorableAssignment( Well::LogInfo, NamedMonitorable )


void Well::LogInfo::copyClassData( const Well::LogInfo& oth )
{
    logunit_ = oth.logunit_;
    dahrg_ = oth.dahrg_;
}


Monitorable::ChangeType Well::LogInfo::compareClassData(
					const Well::LogInfo& oth ) const
{
    if ( name() != oth.name() )
	return cEntireObjectChange();
...
// ----LogInfoSet----
mDefineInstanceCreatedNotifierAccess(Well::LogInfoSet);

Well::LogInfoSet::LogInfoSet()
{
    mTriggerInstanceCreatedNotifier();
}


Well::LogInfoSet::LogInfoSet( const Well::LogInfoSet& oth )
    : NamedMonitorable(oth)
{
    copyClassData( oth );
    mTriggerInstanceCreatedNotifier();
...
mImplMonitorableAssignment( Well::LogInfoSet, NamedMonitorable )


void Well::LogInfoSet::copyClassData( const LogInfoSet& oth )
{
    for ( int idx=0; idx<loginfos_.size(); idx++)
	loginfos_[idx]->unRef();

    loginfos_.erase();
    for ( int idx=0; idx<oth.loginfos_.size(); idx++ )
    {
	Well::LogInfo* newloginfo = new Well::LogInfo( *oth.loginfos_[idx] );
	loginfos_ += newloginfo;
    }
}


Monitorable::ChangeType Well::LogInfoSet::compareClassData(
					const LogInfoSet& oth ) const
{
    if ( loginfos_.size() != oth.loginfos_.size() )
	return cEntireObjectChange();

    for ( int idx=0; idx<loginfos_.size(); idx++ )
	if ( *loginfos_[idx] != *oth.loginfos_[idx] )
	    return cEntireObjectChange();
...
{
    const int sz = loginfos_.size();
    for ( int idx=0; idx<sz; idx++ )
	lognms.add( loginfos_[idx]->name() );
}


Well::LogInfo* Well::LogInfoSet::gtByName( const char* lognm ) const
{
    const int sz = loginfos_.size();
    for ( int idx=0; idx<sz; idx++ )
    {
	const LogInfo& lginfo = *loginfos_[idx];
	if ( lginfo.hasName(lognm) )
	    return const_cast<LogInfo*>( loginfos_[idx] );
    }
...
Well::LogInfoSet::CLogInfoRefMan Well::LogInfoSet::getByName(
						const char* lognm ) const
{
    mLock4Read();
    return gtByName( lognm );
}


Well::LogInfoSet::LogInfoRefMan Well::LogInfoSet::getByName( const char* lognm )
{
    mLock4Read();
    return gtByName( lognm );
}
...
void Well::LogInfoSet::getUnits( BufferStringSet& logunits ) const
{
    const int sz = loginfos_.size();
    for ( int idx=0; idx<sz; idx++ )
	logunits.add( loginfos_[idx]->logunit_ );
}
...
BufferString Well::LogInfoSet::getUnit( const char* lognm ) const
{
    ...
    for ( int idx=0; idx<sz; idx++ )
    {
	if ( loginfos_[idx]->name() == lognm )
	    return loginfos_[idx]->logunit_;
    }

    return "";
}

Well::LogInfoSet::ZIntvType Well::LogInfoSet::dahRange( const char* lognm )
									const
{
    const int sz = loginfos_.size();
    for (int idx=0; idx<sz; idx++)
    {
	if ( loginfos_[idx]->name() == lognm )
	    return loginfos_[idx]->dahrg_;
    }

    return ZIntvType::udf();
}
...
void Well::LogInfoSet::add( Well::LogInfo* loginfo )
{
    mLock4Write();
    loginfo->ref();
    loginfos_.add(loginfo);
}


void Well::LogInfoSet::setEmpty()
{
    if ( isEmpty() )
	return;
...
Well::LogInfoSet::size_type Well::LogInfoSet::size() const
{
    mLock4Read();
