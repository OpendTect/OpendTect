				      ...
				      const Track& track )
{
    ...
    if ( wlidx < 0 )
	return;
    const Log& wl = wd.logs().getLog( wlidx );
...
float Well::LogDataExtracter::calcValWH( const Log& wl, float dah,
				   float winsz, Stats::UpscaleType samppol,
...
static HiddenParam<Well::LogSampler,float>
			logsamplermaxholeszmgr_(mUdf(float));
...
Well::LogSampler::LogSampler( const D2TModel* d2t,
			    const Track* track,
			    const Interval<float>& zrg, bool zrgisintime,
			    float zstep, bool extrintime,
			    Stats::UpscaleType samppol,
...
{
    delete data_;
    logsamplermaxholeszmgr_.removeParam( this );
}


void Well::LogSampler::setMaxHoleSize( float sz )
{
    logsamplermaxholeszmgr_.setParam( this, sz );
}
...
{
    const Log* log = logset_.validIdx( logidx ) ? logset_[logidx] : 0;
    ...
    const float maxholesz = logsamplermaxholeszmgr_.getParam( this );
	...
	lval = LogDataExtracter::calcValWH(*log,dah,winsz,samppol_,maxholesz);
