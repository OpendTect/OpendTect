...
float Well::D2TModel::getTime( float dh, const Track& track ) const
{
    ...
    twt = times.start_ + 2.f* (float)( ( reqz - depths.start_ ) / curvel );
					...
					Interval<float>& times ) const
{
    ...
    depths.start_ = track.getPos(dah_[idah-1]).z_;
    depths.stop_ = track.getPos(dah_[idah]).z_;
    times.start_ = t_[idah-1];
    times.stop_ = t_[idah];
						 ...
						 Interval<float>& times ) const
{
    ...
    depths.start_ = track.getPos( dah_[idah-1] ).z_;
    times.start_ = t_[idah-1];
    depths.stop_ = mUdf(double);
    times.stop_ = mUdf(float);

    for ( int idx=idah; idx<dtsize; idx++ )
    {
        const double curz = track.getPos( dah_[idx] ).z_;
	const float curtwt = t_[idx];
        if ( curz > depths.start_ && curtwt > times.start_ )
	{
            depths.stop_ = curz;
            times.stop_ = curtwt;
	    break;
    ...
    if ( mIsUdf(times.stop_) ) // found nothing good below, search above
    {
        depths.stop_ = depths.start_;
        times.stop_ = times.start_;
        depths.start_ = mUdf(double);
        times.start_ = mUdf(float);
	for ( int idx=idah-2; idx>=0; idx-- )
	{
            const double curz = track.getPos( dah_[idx] ).z_;
	    const float curtwt = t_[idx];
            if ( curz < depths.stop_ && curtwt < times.stop_ )
	    {
                depths.start_ = curz;
                times.start_ = curtwt;
		break;
				    ...
				    float replvel )
{
    ...
    int idahofminz = 0;
    int idahofmaxz = 0;
    double tvdmin = track.pos(0).z_;
    double tvdmax = track.pos(0).z_;

    for ( int idx=1; idx<track.size(); idx++ )
    {
        const double zpostrack = track.pos(idx).z_;
	if ( zpostrack > tvdmax )
	{
	    tvdmax = zpostrack;
	    idahofmaxz = idx;
					...
					TimeDepthModel& model ) const
{
	...
	const float depth = getConvertedValue(
                                mCast(float,track.getPos(curdah).z_ ),
				...
				const TypeSet<double>& tvals, double wllheadz )
{
	...
	if ( !velrg.overlaps(vrepldepthrg) )
        ...
        if ( velrg.start_ > vrepldepthrg.stop_ )
	    break;
					       ...
					       uiString& msg )
{
    ...
    if ( !mIsEqual((float)vreplinfile,info.replvel_,1e-1f) )
    {
				     ...
				     uiString& errmsg, uiString& warnmsg )
{
    ...
    if ( !truncateToTD(zvals,tvals,trackrg.stop_) )
