...
{
    return sobody_ ? sobody_->isCameraPerspective() : false;
...
{
    return sobody_ && sobody_->getScene() ? sobody_->getScene()->id() : -1;
}
...
Geom::Size2D<int> ui3DViewer::getViewportSizePixels() const
{
    SbVec2s size( 0, 0 );
    if ( sobody_ )
	 size = sobody_->getViewportRegion().getViewportSizePixels();
