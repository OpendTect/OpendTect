...
template<>
void EnumDefImpl<Mnemonic::Scale>::init()
{
    uistrings_ += uiStrings::sLinear();
    uistrings_ += uiStrings::sLogarithmic();
}
...
void Mnemonic::fillPar( IOPar& iop ) const
{
    ...
    fms += PropertyRef::StdTypeDef().getKey( stdType() );
    fms += Mnemonic::ScaleDef().getKey( disp_.scale_ );
...
MnemonicSetMgr()
    : mns_( nullptr )
{
    DBM().surveyChanged.notify( mCB(this,MnemonicSetMgr,doNull) );
}
