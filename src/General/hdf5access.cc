...
BufferString HDF5::DataSetKey::fullDataSetName() const
{
    ...
    return fp.fullPath( File::Path::Unix );
}
...
{
    return sHDF5NotAvailable().appendPhrase(
				    tr("Needed to access '%1'").arg(fnm) );
}
...
HDF5::ODDataType HDF5::Reader::getDataType( const DataSetKey& dsky,
				      uiRetVal& uirv ) const
{
    const H5::DataSet* scope = getDSScope( dsky );
    if ( !scope )
	return OD::F32;

    return gtDataType( *scope );
}
				      ...
				      const ArrayNDInfo& inf )
{
	...
	{ pErrMsg("zero dims"); }
    H5::DataSet* scope = setDSScope( dsky );
    if ( !scope )
	mRetNoScopeInUiRv()

    reSzDS( inf, *scope, uirv );
    return uirv;
}
