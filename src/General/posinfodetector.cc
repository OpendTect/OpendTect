...
void PosInfo::Detector::report( StringPairSet& report ) const
{
    ...
    report.add( "X-Coordinate range",
		getRangeStr(mincoord_.x,maxcoord_.x,SI().nrXYDecimals()) );
    report.add( "Y-Coordinate range",
		getRangeStr(mincoord_.y,maxcoord_.y,SI().nrXYDecimals()) );
    ...
    if ( setup_.isps_ )
    {
	report.add( "Offsets", getRangeStr(offsrg_.start,offsrg_.stop,2) );
	report.add( "Azimuths",
		    getRangeStr(azimuthrg_.start,azimuthrg_.stop,2) );
	if ( mIsUdf(firstaltnroffs_.binid_.inl()) )
