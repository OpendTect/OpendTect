...
bool HorVariogramComputer::compVarFromRange( DataPointSet& dpset, int size,
					    int cid, int range, int fold,
					    uiString& errmsg, bool msgiserror )
{
    ...
    {
	errmsg = tr("Failed to compute the total variance");
	errmsg.appendPhrase( tr("Please check the input data") );
	msgiserror = true;
	return false;
    }
...
//    ExecutorGroup computer( "Processing variogram" );
	...
	for( int ilag=1; ilag<size; ilag++ )
	{
	    ...
	    if ( maxinl < mininl && icomp != 0 )
		continue;

	    if ( maxcrl < mincrl && icomp != 2 )
		continue;
					    ...
					    uiString& errmsg, bool msgiserror )
{
	...
	errmsg.appendPhrase( tr("Could not retrieve group names") );
	errmsg.appendPhrase( tr("Will use generic names") );
	msgiserror = false;
    }
	...
	for ( int idz = 0; idz<nrin-1; idz++ )
	{
		...
		errmsg.appendPhrase(
		    tr("Please re-extract with Radius around wells = 0") );
		msgiserror = true;
		return false;
	    }
	}
	     ...
	     (zbase - ztop) < 3*step )
	{
	    ...
	    errmsg.appendPhrase( tr("Well %1 is not used").
	...
	errmsg.appendPhrase( tr("Collect more data or decrease fold") );
	msgiserror = true;
	return false;
    }
	...
	if ( nrwells > 1 )
	    errmsg.appendPhrase(
	    tr("Only %1 out of %2 wells contributed to the output")
	    .arg(nrcontribwells).arg(nrwells), uiString::NoSep);
	else
	    errmsg.appendPhrase(
		tr("Only %1 out of %2 well contributed to the output")
		.arg(nrcontribwells).arg(nrwells), uiString::NoSep);
