...
{
    const bool velinfeetsec = desc.getUnit() == UnitOfMeasure::feetSecondUnit();
			  ...
			  const bool* velinfeetsec =nullptr )
{
    ...
    suffix.add( " and velocities in " ).add( descin.getUnit()->symbol() );
    ...
    suffix.add( "; within project in " )
	  .add( zinfo.isTime() ? "time"
		  : (zinfo.isDepthMeter() ? "depth-meter"
					  : "depth-feet") )
	  .add( " and velocities in " ).add( desc.getUnit()->symbol() );
