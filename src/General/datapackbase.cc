...
public:
	...
	hsamp_.set( StepInterval<int>((int)tmpirg.start,(int)tmpirg.stop,
		...
		StepInterval<int>((int)tmpcrg.start,(int)tmpcrg.stop,
		    (int)tmpcrg.step) );
	const Coord spt1 =
		SI().transform( BinID(hsamp_.start.inl(),hsamp_.start.crl()) );
	const Coord spt2 =
		SI().transform( BinID(hsamp_.start.inl(),hsamp_.stop.crl()) );
	const Coord spt3 =
		SI().transform( BinID(hsamp_.stop.inl(),hsamp_.start.crl()) );
	const Coord spt4 =
		SI().transform( BinID(hsamp_.stop.inl(),hsamp_.stop.crl()) );
		...
		idx++, iter.next(), addToNrDone(1) )
	{
		...
		const int id0v00 =
		    (approxbid.inl()-hsamp_.start.inl()) / hsamp_.step.inl()
		    + toreach00.inl();
		const int id1v00 =
		    (approxbid.crl()-hsamp_.start.crl()) / hsamp_.step.crl()
		    + toreach00.crl();
