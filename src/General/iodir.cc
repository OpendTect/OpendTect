...
static HiddenParam<IODir,od_int64> hp_lastmodtime(0);
...
IODir::IODir( const char* dirnm )
    ...
    , curid_(0)
{
    hp_lastmodtime.setParam( this, 0 );
    if ( build() )
	isok_ = true;
}
...
IODir::~IODir()
{
    hp_lastmodtime.removeParam( this );
    deepErase( objs_ );
}
...
void IODir::update()
{
    const od_int64 lastmodtime = hp_lastmodtime.getParam( this );
...
bool IODir::doWrite() const
{
    ...
    hp_lastmodtime.setParam( cCast(IODir*,this),
			     File::getTimeInSeconds(filenm) );
    return true;
}
