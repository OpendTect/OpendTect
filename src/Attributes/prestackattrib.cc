...
static HiddenParam< PSAttrib, int > anglegsdpids( -1 );
...
#define mGetSetupEnumPar(var,typ) \
    ...
    anglegsdpids.setParam( this, anglegsdpid_ );
...
{
    anglegsdpids.setParam( this, angledpid );
}
...
bool PSAttrib::getInputData( const BinID& relpos, int zintv )
{
	...
	const int anglegsdpid_ = anglegsdpids.getParam( this );
	if ( anglegsdpid_>= 0 )
	{
	    ConstDataPackRef<DataPack> angledp =
		DPM( DataPackMgr::CubeID() ).obtain( anglegsdpid_ );
