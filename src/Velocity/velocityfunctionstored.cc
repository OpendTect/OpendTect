...
-*/
static const char* rcsID = "$Id: velocityfunctionstored.cc,v 1.1 2009-03-18 18:45:26 cvskris Exp $";
...
bool StoredFunction::moveTo( const BinID& bid )
{
    ...
    sort_coupled( zval_.arr(), idxs, zval_.size() );
       				      ...
       				      float* res ) const
{
	...
	return sampleVint( vel_.arr(), zval_.arr(), zval_.size(),
		    getDesc().samplespan_, SamplingData<double>( z0, dz ),
		    res, nr );
    }
