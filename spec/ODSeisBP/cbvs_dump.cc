...
bool BatchProgram::go( std::ostream& logstrm )
{
    ...
    ioobj.setTranslator( mTranslatorKey(SeisTrc,CBVS) );
    CubeSampling datacs;
	...
	if ( inlselstr || crlselstr )
	{
		...
		if ( sz > 0 ) cs.hrg.start.inl = fms.getIValue( 0 );
		if ( sz > 1 ) cs.hrg.stop.inl = fms.getIValue( 1 );
	    }
	    if ( crlselstr )
	    {
		fms = crlselstr;
		const int sz = fms.size();
		if ( sz > 0 ) cs.hrg.start.crl = fms.getIValue( 0 );
		if ( sz > 1 ) cs.hrg.stop.crl = fms.getIValue( 1 );
	    }
	}
	if ( zselstr )
	{
	    ...
	    if ( sz > 0 ) cs.zrg.start = fms.getFValue( 0 );
	    if ( sz > 1 ) cs.zrg.stop = fms.getFValue( 1 );
	    if ( SI().zIsTime() )
